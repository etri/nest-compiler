 ...
 */
#ifndef GLOW_BACKENDS_Relay_RelayDEVICEMANAGER_H
#define GLOW_BACKENDS_Relay_RelayDEVICEMANAGER_H
...
/// A class controlling a single "Relay Device", a thread of execution in
/// the IR-Relay. Many RelayFunctions may be added, but only one
/// inference is executed at a time.
class RelayDeviceManager : public QueueBackedDeviceManager {
...
public:
  explicit RelayDeviceManager(const DeviceConfig &config)
    ...
    statsExporterRegistry_->incrementCounter(kDevicesUsedRelay);
    exportMemoryCounters();
  }

  ~RelayDeviceManager() override {
    statsExporterRegistry_->incrementCounter(kDevicesUsedRelay, -1);
    zeroMemoryCounters();
  }
...
#endif // GLOW_BACKENBDS_Relay_RelayDEVICEMANAGER_H
