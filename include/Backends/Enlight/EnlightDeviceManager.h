 ...
 */
#ifndef NESTC_BACKENDS_NEWTON_NEWTONDEVICEMANAGER_H
#define NESTC_BACKENDS_NEWTON_NEWTONDEVICEMANAGER_H
...
/// A class controlling a single "Enlight Device", a thread of execution in
/// the IR-Enlight. Many EnlightFunctions may be added, but only one
/// inference is executed at a time.
class EnlightDeviceManager : public QueueBackedDeviceManager {
  ...
  static constexpr const char *kDevicesUsedEnlight =
      "glow.devices_used.interpreter";

 public:
  explicit EnlightDeviceManager(const DeviceConfig &config)
      : QueueBackedDeviceManager(config) {
    statsExporterRegistry_->incrementCounter(kDevicesUsedEnlight);
    exportMemoryCounters();
  }

  ~EnlightDeviceManager() override {
    statsExporterRegistry_->incrementCounter(kDevicesUsedEnlight, -1);
    zeroMemoryCounters();
  }
...
#endif // NESTC_BACKENDS_NEWTON_NEWTONDEVICEMANAGER_H
