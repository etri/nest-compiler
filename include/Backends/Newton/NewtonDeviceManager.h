...
/// A class controlling a single "Newton Device", a thread of execution in
/// the IR-Newton. Many NewtonFunctions may be added, but only one
/// inference is executed at a time.
class NewtonDeviceManager : public QueueBackedDeviceManager {
  ...
  static constexpr const char *kDevicesUsedNewton =
      "glow.devices_used.interpreter";

 public:
  explicit NewtonDeviceManager(const DeviceConfig &config)
      : QueueBackedDeviceManager(config) {
    statsExporterRegistry_->incrementCounter(kDevicesUsedNewton);
    exportMemoryCounters();
  }

  ~NewtonDeviceManager() override {
    statsExporterRegistry_->incrementCounter(kDevicesUsedNewton, -1);
    zeroMemoryCounters();
  }
