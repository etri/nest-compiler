...
/// Function "compiled" for execution by the Newton.
class NewtonFunction final : public CompiledFunction,
 ...
 public:
  NewtonFunction(std::unique_ptr<IRFunction> F,
                 runtime::RuntimeBundle &&bundle);

  /// \name CompiledFunction interface
  ///@{
  ~NewtonFunction() override;
...
/// An NewtonFunction bound to a specific invocation.
class BoundNewtonFunction : public IRInstructionProcessingHandler {
  ...
  /// A reference to the constant map from the owning NewtonFunction.
  const std::unordered_map<std::string, Tensor *> &constants_;

 public:
  explicit BoundNewtonFunction(
 ...
 private:
  ...
  /// @name BoundNewtonFunction methods. This is a list of method
  /// declerations that are used by the Newton to dispatch different
  /// instructions.
  ///@{
