...
class NestPartitioner final : public PartitionerBase {
  ...
  std::vector<NodeGroup *> partitionList_;
  std::set<std::string> backendSet_;
  std::string quantFileName_;

  std::map<std::string, std::map<std::string, float>> backendExeProfileMap_,
...
public:
  ...
  Expected<DAGListTy> partition(CompilationContext &cctx, size_t exeType,
  ...
  void outPartitionPlanForSingleNode(Function *function,
                                     ...
                                     std::string filename);
  void outPerformProfileForFusedNode(std::vector<NodeGroup *> *branchList,
  ...
  void loadPartitionPlan(runtime::PartitionConfig *partitionConfig,
                         std::string filename);
  ...
  void getBranch(std::vector<std::vector<CostNode>> *cnodebfs, BFSLevel dag,
                 std::vector<Node *> parallelNodes, Node *lastNode,
                 NodeGroup *prevBranch, unsigned int level,
                 unsigned int branchID, std::vector<NodeGroup *> *branchList);
  void backtrack(std::vector<std::vector<CostNode>> *cnodebfs, BFSLevel dag,
                 Node *lastNode, NodeGroup *branch, unsigned int level,
                 unsigned int branchID, std::vector<NodeGroup *> *branchList);
  void analyzeDAG(std::vector<std::vector<CostNode>> *cnodebfs, BFSLevel dag,
                  std::vector<NodeGroup *> *branchList);
  bool setBranchPU(NodeGroup *branch);
  void setCost(std::vector<Backend *> *backends, CostNode *pnode,
               CostNode *cnode);
  void makeCostNode(Function *function,
                    std::vector<std::vector<CostNode>> *cnodeBfs);
  bool isSupportedOpType(Kinded::Kind opType, std::string backendName,
                         CostNode *cnode);
