#include "vtaResConv3TestMainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
#define DEFAULT_WIDTH 56
#define DEFAULT_CHANNEL 64
#define DEFAULT_OHEIGHT 28
#define DEFAULT_OWIDTH 28
#define DEFAULT_OCHANNEL 128
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaResConv3TestBundle.weights.bin", vtaResConv3TestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaResConv3TestBundle_config);
  int8_t *activationsAddr = initActivations(vtaResConv3TestBundle_config);

  freopen("ResConv3Test.csv", "w", stdout);
  printf("1,56,56,64,128,3,3,1,2,0,1,7,28,28,%d,%d,%d", input1, input2, input3);
  fclose(stdout);

  freopen("ResConv3Test.csv", "a+", stdout);
  printf(",");

  vtaResConv3TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

  // Perform the computation.
  int errCode =
      vtaResConv3TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr, input1, input2, input3);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }
  
  // Report the results.
  dumpInferenceResults(vtaResConv3TestBundle_config, mutableWeightVarsAddr);
  vtaResConv3TestMainEntry_destroy_module();
