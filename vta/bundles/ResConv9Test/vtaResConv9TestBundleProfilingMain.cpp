#include "vtaResConv9TestMainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaResConv9TestBundle.weights.bin", vtaResConv9TestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaResConv9TestBundle_config);
  int8_t *activationsAddr = initActivations(vtaResConv9TestBundle_config);

  freopen("ResConv9Test.csv", "w", stdout);
  printf("1,14,14,256,512,3,3,1,2,0,1,8,7,7,%d,%d,%d", input1, input2, input3);
  fclose(stdout);

  freopen("ResConv9Test.csv", "a+", stdout);
  printf(",");

  vtaResConv9TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

  // Perform the computation.
  int errCode =
      vtaResConv9TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr, input1, input2, input3);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }
  
  // Report the results.
  dumpInferenceResults(vtaResConv9TestBundle_config, mutableWeightVarsAddr);
  vtaResConv9TestMainEntry_destroy_module();
