#include "vtaResConv6TestMainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaResConv6TestBundle.weights.bin", vtaResConv6TestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaResConv6TestBundle_config);
  int8_t *activationsAddr = initActivations(vtaResConv6TestBundle_config);

  freopen("ResConv6Test.csv", "w", stdout);
  printf("1,28,28,128,256,3,3,1,2,0,1,8,14,14,%d,%d,%d", input1, input2, input3);
  fclose(stdout);

  freopen("ResConv6Test.csv", "a+", stdout);
  printf(",");

  vtaResConv6TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

  // Perform the computation.
  int errCode =
      vtaResConv6TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr, input1, input2, input3);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }
  
  // Report the results.
  dumpInferenceResults(vtaResConv6TestBundle_config, mutableWeightVarsAddr);
  vtaResConv6TestMainEntry_destroy_module();
