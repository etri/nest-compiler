#include "vtaResConv8TestMainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaResConv8TestBundle.weights.bin", vtaResConv8TestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaResConv8TestBundle_config);
  int8_t *activationsAddr = initActivations(vtaResConv8TestBundle_config);

  freopen("ResConv8Test.csv", "w", stdout);
  printf("1,14,14,256,512,1,1,0,2,0,1,5,7,7,%d,%d,%d", input1, input2, input3);
  fclose(stdout);

  freopen("ResConv8Test.csv", "a+", stdout);
  printf(",");

  vtaResConv8TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

  // Perform the computation.
  int errCode =
      vtaResConv8TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr, input1, input2, input3);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }
  
  // Report the results.
  dumpInferenceResults(vtaResConv8TestBundle_config, mutableWeightVarsAddr);
  vtaResConv8TestMainEntry_destroy_module();
