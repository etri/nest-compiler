...
int main(int argc, char **argv) {
  ...
  int errCode =
          vtaElemAddTestMainEntry(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaElemAddTestBundle_config, mutableWeightVarsAddr);
