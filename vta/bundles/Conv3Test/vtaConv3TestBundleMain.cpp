...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaConv3TestBundle.weights.bin", vtaConv3TestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaConv3TestBundle_config);
  int8_t *activationsAddr = initActivations(vtaConv3TestBundle_config);

  vtaConv3TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

  // Perform the computation.
  int errCode =
      vtaConv3TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaConv3TestBundle_config, mutableWeightVarsAddr);

  vtaConv3TestMainEntry_destroy_module();
  // Free all resources.
