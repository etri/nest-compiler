...
static int8_t *initMutableWeightVars(const BundleConfig &config) {
  ...
  size_t imageDataSizeInBytes =
          inputDims[0] * inputDims[1] * inputDims[2] * sizeof(int8_t);
...
int main(int argc, char **argv) {
  ...
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaAvgPoolTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaAvgPoolTestBundle_config);

  // Perform the computation.
  int errCode =
          vtaAvgPoolTestMainEntry((uint8_t *)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaAvgPoolTestBundle_config, mutableWeightVarsAddr);
