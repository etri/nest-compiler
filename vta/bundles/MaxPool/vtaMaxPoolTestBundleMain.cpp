...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaMaxPoolTestBundle.weights.bin", vtaMaxPoolTestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaMaxPoolTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaMaxPoolTestBundle_config);

  // Perform the computation.
  int errCode =
          vtaMaxPoolTestMainEntry(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaMaxPoolTestBundle_config, mutableWeightVarsAddr);
