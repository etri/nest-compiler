...
//  printf( "inputDataVar.offset = %d\n", inputDataVar.offset);
  //memcpy(mutableWeightVarsAddr + inputDataVar.offset, inputT, inputDataVar.size*4);
  memcpy(mutableWeightVarsAddr + inputDataVar.offset, inputT, inputDataVar.size*4);
...
int main(int argc, char **argv) {
  ...
  uint8_t *constantWeightVarsAddr = (uint8_t *) initConstantWeights(
      "BirealNet.weights.bin", BirealNet_config);

  uint8_t *mutableWeightVarsAddr =
      (uint8_t *) initMutableWeightVars(BirealNet_config, "input");
  uint8_t *activationsAddr =
      (uint8_t *) initActivations(BirealNet_config);

  double result;
  timeval t1, t2;
  BirealNet_load_module(constantWeightVarsAddr);
  BirealNet(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);

  if(REPEAT_NUM>1) {
    usleep(1000000);
    for (int i = 0; i < REPEAT_NUM; i++) {
      BirealNet(constantWeightVarsAddr, mutableWeightVarsAddr,
                              activationsAddr);
  ...
  BirealNet_destroy_module();
  // Report the results.
  int maxIdx = dumpInferenceResults(BirealNet_config, (uint8_t*) mutableWeightVarsAddr, "output");
