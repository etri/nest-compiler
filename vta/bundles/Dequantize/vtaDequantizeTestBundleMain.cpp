...
int main(int argc, char **argv) {
  ...
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaDequantizeTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaDequantizeTestBundle_config);

  // Perform the computation.
  int errCode =
          vtaDequantizeTestMainEntry(0, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaDequantizeTestBundle_config, mutableWeightVarsAddr);
