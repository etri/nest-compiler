#include "vtaConv2TestMainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaConv2TestBundle.weights.bin", vtaConv2TestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaConv2TestBundle_config);
  int8_t *activationsAddr = initActivations(vtaConv2TestBundle_config);

  vtaConv2TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

  // Perform the computation.
  int errCode =
      vtaConv2TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaConv2TestBundle_config, mutableWeightVarsAddr);

  vtaConv2TestMainEntry_destroy_module();
  // Free all resources.
