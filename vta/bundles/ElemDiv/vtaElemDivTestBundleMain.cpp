...
static int8_t *initMutableWeightVars(const BundleConfig &config) {
  ...
  size_t imageDataSizeInBytes =
          inputDims[0] * inputDims[1] * inputDims[2] * inputDims[3] * sizeof(int8_t) * 2;
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaElemDivTestBundle.weights.bin", vtaElemDivTestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaElemDivTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaElemDivTestBundle_config);

  // Perform the computation.
  int errCode =
          vtaElemDivTestMainEntry(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaElemDivTestBundle_config, mutableWeightVarsAddr);
