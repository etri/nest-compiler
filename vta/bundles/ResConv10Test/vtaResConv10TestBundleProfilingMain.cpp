...
#include "vtaResConv10TestMainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
#define DEFAULT_CHANNEL 512
#define DEFAULT_OHEIGHT 7
#define DEFAULT_OWIDTH 7
#define DEFAULT_OCHANNEL 512
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaResConv10TestBundle.weights.bin", vtaResConv10TestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaResConv10TestBundle_config);
  int8_t *activationsAddr = initActivations(vtaResConv10TestBundle_config);

  freopen("ResConv10Test.csv", "w", stdout);
  printf("1,7,7,512,512,3,3,1,1,0,1,7,7,7,%d,%d,%d", input1, input2, input3);
  fclose(stdout);

  freopen("ResConv10Test.csv", "a+", stdout);
  printf(",");

  vtaResConv10TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

  // Perform the computation.
  int errCode =
      vtaResConv10TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr, input1, input2, input3);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }
  
  // Report the results.
  dumpInferenceResults(vtaResConv10TestBundle_config, mutableWeightVarsAddr);
  vtaResConv10TestMainEntry_destroy_module();
