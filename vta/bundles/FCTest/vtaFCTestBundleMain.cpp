...
static int8_t *initMutableWeightVars(const BundleConfig &config) {
  ...
  size_t imageDataSizeInBytes =
          inputDims[0] * inputDims[1] * sizeof(int8_t);
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaFCTestBundle.weights.bin", vtaFCTestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaFCTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaFCTestBundle_config);

  // Perform the computation.
  int errCode =
          vtaFCTestMainEntry(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaFCTestBundle_config, mutableWeightVarsAddr);
