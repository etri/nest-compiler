#include "vtaConv4TestMainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
int main(int argc, char **argv) {
    ...
    int8_t *constantWeightVarsAddr =
            initConstantWeights("vtaConv4TestBundle.weights.bin", vtaConv4TestBundle_config);
    int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaConv4TestBundle_config);
    int8_t *activationsAddr = initActivations(vtaConv4TestBundle_config);

    vtaConv4TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

    // Perform the computation.
    int errCode =
            vtaConv4TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr);
    if (errCode != GLOW_SUCCESS) {
        printf("Error running bundle: error code %d\n", errCode);
    }

    // Report the results.
    dumpInferenceResults(vtaConv4TestBundle_config, mutableWeightVarsAddr);

    vtaConv4TestMainEntry_destroy_module();
    // Free all resources.
