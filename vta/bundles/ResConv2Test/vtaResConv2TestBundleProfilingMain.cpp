...
#include "vtaResConv2TestMainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaResConv2TestBundle.weights.bin", vtaResConv2TestBundle_config);
  ...
  int errCode =
      vtaResConv2TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr, input1, input2, input3);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }
  
  // Report the results.
  dumpInferenceResults(vtaResConv2TestBundle_config, mutableWeightVarsAddr);
  vtaResConv2TestMainEntry_destroy_module();
