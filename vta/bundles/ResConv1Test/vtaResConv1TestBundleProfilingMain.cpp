...
#include "vtaResConv1TestMainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
#define DEFAULT_OHEIGHT 56
#define DEFAULT_OWIDTH 56
#define DEFAULT_OCHANNEL 64
...
int main(int argc, char **argv) {
    ...
    int8_t *constantWeightVarsAddr =
            initConstantWeights("vtaResConv1TestBundle.weights.bin", vtaResConv1TestBundle_config);
    int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaResConv1TestBundle_config);
    int8_t *activationsAddr = initActivations(vtaResConv1TestBundle_config);

    freopen("ResConv1Test.csv", "w", stdout);
    printf("1,56,56,64,64,3,3,1,1,0,1,7,56,56,%d,%d,%d", input1, input2, input3);
    fclose(stdout);

    freopen("ResConv1Test.csv", "a+", stdout);
    printf(",");

    vtaResConv1TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

    // Perform the computation.
    int errCode =
            vtaResConv1TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr, input1, input2, input3);
    if (errCode != GLOW_SUCCESS) {
        printf("Error running bundle: error code %d\n", errCode);
    }

    // Report the results.
    dumpInferenceResults(vtaResConv1TestBundle_config, mutableWeightVarsAddr);

    vtaResConv1TestMainEntry_destroy_module();
