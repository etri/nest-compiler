    ...
    for (int f = 0; f < filter_size; f++){
      for (int i = 0; i<out_h; i++){	//h
        for (int j = 0; j < out_w; j++){	//w
          ...
          int8_t v = *(results+(out_ch*out_h*out_w*16 + i*out_w*16 + j*16 + oo));
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaConvTestBundle.weights.bin", vtaConvTestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaConvTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaConvTestBundle_config);

  vtaConvTestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

  // Perform the computation.
  int errCode =
      vtaConvTestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaConvTestBundle_config, mutableWeightVarsAddr);

  vtaConvTestMainEntry_destroy_module();
  // Free all resources.
