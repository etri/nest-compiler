#include "vtaResConv4TestMainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaResConv4TestBundle.weights.bin", vtaResConv4TestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaResConv4TestBundle_config);
  int8_t *activationsAddr = initActivations(vtaResConv4TestBundle_config);

  freopen("ResConv4Test.csv", "w", stdout);
  printf("1,28,28,128,128,3,3,1,1,0,1,7,28,28,%d,%d,%d", input1, input2, input3);
  fclose(stdout);

  freopen("ResConv4Test.csv", "a+", stdout);
  printf(",");

  vtaResConv4TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

  // Perform the computation.
  int errCode =
      vtaResConv4TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr, input1, input2, input3);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }
  
  // Report the results.
  dumpInferenceResults(vtaResConv4TestBundle_config, mutableWeightVarsAddr);
  vtaResConv4TestMainEntry_destroy_module();
