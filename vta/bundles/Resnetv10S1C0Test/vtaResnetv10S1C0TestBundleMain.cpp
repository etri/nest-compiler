...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaResnetv10S1C0TestBundle.weights.bin", vtaResnetv10S1C0TestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaResnetv10S1C0TestBundle_config);
  int8_t *activationsAddr = initActivations(vtaResnetv10S1C0TestBundle_config);

  vtaResnetv10S1C0TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

  // Perform the computation.
  int errCode =
      vtaResnetv10S1C0TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaResnetv10S1C0TestBundle_config, mutableWeightVarsAddr);

  vtaResnetv10S1C0TestMainEntry_destroy_module();
  // Free all resources.
