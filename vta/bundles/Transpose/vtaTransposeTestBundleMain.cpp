...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaTransposeTestBundle.weights.bin", vtaTransposeTestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaTransposeTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaTransposeTestBundle_config);

  // Perform the computation.
  int errCode =
          vtaTransposeTestMainEntry(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaTransposeTestBundle_config, mutableWeightVarsAddr);
