...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaElemSubTestBundle.weights.bin", vtaElemSubTestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaElemSubTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaElemSubTestBundle_config);

  // Perform the computation.
  int errCode =
          vtaElemSubTestMainEntry(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaElemSubTestBundle_config, mutableWeightVarsAddr);
