...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaNonVTAConvTestBundle.weights.bin", vtaNonVTAConvTestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaNonVTAConvTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaNonVTAConvTestBundle_config);

  // Perform the computation.
  int errCode =
          vtaNonVTAConvTestMainEntry(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaNonVTAConvTestBundle_config, mutableWeightVarsAddr);
