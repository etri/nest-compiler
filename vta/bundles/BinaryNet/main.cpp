    ...
    for (unsigned z = 0; z < 3; z++) {
      for (unsigned y = 0; y < dims[1]; y++) {
        for (unsigned x = 0; x < dims[0]; x++) {
            //resultT[getXYZ(dims, 2 - z, y, z)] =
            resultT[getXYZW(resultDims, n, 2 - z, x, y)] =
              (imageT[getXYZ(dims, x, y, z)] - 0.5) / 0.5;
        }
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  uint8_t *mutableWeightVarsAddr =
      (uint8_t *) initMutableWeightVars(binarynet_config, "input");
  uint8_t *activationsAddr =
      (uint8_t *) initActivations(binarynet_config);
