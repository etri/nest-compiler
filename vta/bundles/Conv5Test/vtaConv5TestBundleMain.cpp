...
int main(int argc, char **argv) {
    ...
    int8_t *constantWeightVarsAddr =
            initConstantWeights("vtaConv5TestBundle.weights.bin", vtaConv5TestBundle_config);
    int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaConv5TestBundle_config);
    int8_t *activationsAddr = initActivations(vtaConv5TestBundle_config);

    vtaConv5TestMainEntry_load_module((uint8_t*)constantWeightVarsAddr);

    // Perform the computation.
    int errCode =
            vtaConv5TestMainEntry((uint8_t*)constantWeightVarsAddr, (uint8_t*)mutableWeightVarsAddr, (uint8_t*)activationsAddr);
    if (errCode != GLOW_SUCCESS) {
        printf("Error running bundle: error code %d\n", errCode);
    }

    // Report the results.
    dumpInferenceResults(vtaConv5TestBundle_config, mutableWeightVarsAddr);

    vtaConv5TestMainEntry_destroy_module();
    // Free all resources.
