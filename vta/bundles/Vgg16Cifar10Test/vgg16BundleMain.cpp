...
#define DEFAULT_HEIGHT 32 //224
#define DEFAULT_WIDTH 32 //224
#define REPEAT_NUM 1
//===----------------------------------------------------------------------===//
...
//         imageDataSizeInBytes);
  const SymbolTableEntry &inputGPUDataVar = getMutableWeightVar(config, varName);
  if(NHWC == 1) {
...
int main(int argc, char **argv) {
  ...
  uint8_t *constantWeightVarsAddr = (uint8_t *) initConstantWeights(
      "vgg16_cifar10.weights.bin", vgg16_cifar10_config);

  uint8_t *mutableWeightVarsAddr =
      (uint8_t *) initMutableWeightVars(vgg16_cifar10_config, "input");
  uint8_t *activationsAddr =
      (uint8_t *) initActivations(vgg16_cifar10_config);
  
  double result;
  clock_t start, end;
  vgg16_cifar10_load_module(constantWeightVarsAddr);
//  vgg16(constantWeightVarsAddr, mutableWeightVarsAddr,
//                          activationsAddr);

  if(REPEAT_NUM>1) {
    usleep(1000000);
    for (int i = 0; i < REPEAT_NUM; i++) {
      vgg16_cifar10(constantWeightVarsAddr, mutableWeightVarsAddr,
                              activationsAddr);
  ...
  vgg16_cifar10_destroy_module();
  end = clock();

  // Report the results.
  int maxIdx = dumpInferenceResults(vgg16_cifar10_config, (uint8_t*) mutableWeightVarsAddr,
                                    "output");
