...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaSplatMaxTestBundle.weights.bin", vtaSplatMaxTestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaSplatMaxTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaSplatMaxTestBundle_config);

  // Perform the computation.
  int errCode =
          vtaSplatMaxTestMainEntry(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaSplatMaxTestBundle_config, mutableWeightVarsAddr);
