...
static void dumpInferenceResults(const BundleConfig &config,
  ...
  const SymbolTableEntry &outputWeights =
          getMutableWeightVar(config, "results");
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("vtaTestBundle.weights.bin", vtaTestBundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(vtaTestBundle_config);
  int8_t *activationsAddr = initActivations(vtaTestBundle_config);

  // Perform the computation.
  int errCode =
          vtaTestMainEntry(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(vtaTestBundle_config, mutableWeightVarsAddr);
