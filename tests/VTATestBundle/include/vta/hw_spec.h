...
#define VTA_WGT_BUFF_DEPTH (VTA_WGT_BUFF_SIZE / VTA_WGT_ELEM_BYTES * VTA_BUFFER_NTIMES)
...
#define VTA_INP_BUFF_DEPTH (VTA_INP_BUFF_SIZE / VTA_INP_ELEM_BYTES * VTA_BUFFER_NTIMES)
...
#define VTA_ACC_BUFF_DEPTH (VTA_ACC_BUFF_SIZE / VTA_ACC_ELEM_BYTES * VTA_BUFFER_NTIMES)
...
/*! Mem ID constant: sram move */
#define VTA_MEM_ID_MOVE 5
...
typedef struct {
  ...
  uint64_t dst_idx    : VTA_LOG_ACC_BUFF_DEPTH;
  /*! \brief Source index (indexes input buffer for GEMM or accum buffer for ALU) */
  uint64_t src_idx    : VTA_LOG_INP_BUFF_DEPTH;
  /*! \brief Weight index (indexes weight buffer) */
  uint64_t wgt_idx    : VTA_LOG_WGT_BUFF_DEPTH;
} VTAUop;
