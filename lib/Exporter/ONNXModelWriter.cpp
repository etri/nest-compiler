...
Error ONNXModelWriter::writeTranspose(const TransposeNode *node,
  ...
  if (llvm::dyn_cast<ConvolutionNode>(input) ||
      llvm::dyn_cast<BNNConvolutionNode>(input) ||
...
Error ONNXModelWriter::writeBNNConvolution(const BNNConvolutionNode *node,
                                        GraphType &graph) {
  ...
  proto->add_input(node->getScalingfactor().getNode()->getName());
  proto->set_name(node->getName());
  proto->set_op_type("BNNConv");
...
DEF_ALL_WRITER_NODE(TFLiteCustomOperator)
