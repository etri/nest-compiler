...
static void toBinImpl(Handle<ElemTy> handle, llvm::raw_ostream &os, const char* name) {
    char filename[100];
...
static void toBinImpl_int32(Handle<int32_t> handle, llvm::raw_ostream &os, const char* name) {
    char filename[100];
    assert(handle.size() %2 == 0);
    strcpy(filename,name);
    strcat(filename,".bin");
...
static void toBinImpl_float(Handle<float> handle, llvm::raw_ostream &os, const char* name) {
    char filename[100];
    strcpy(filename,name);
    strcat(filename,".bin");
...
void Tensor::toBin(const char *name) const {
    auto T = this;
    //assert(T->getElementType() == ElemKind::Int8QTy ||T->getElementType() == ElemKind::Int32QTy );
