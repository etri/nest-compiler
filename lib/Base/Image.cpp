...
bool useCifar10Normalization;
static llvm::cl::opt<bool, true> useCifar10NormalizationF(
    "use-cifar10-normalization",
    llvm::cl::desc("Use Cifar10 Normalization. This works in combination "
                   "with the Image Mode normalization."),
    llvm::cl::cat(imageCat), llvm::cl::location(useCifar10Normalization),
    llvm::cl::init(false));
...
void glow::readPngPpmImagesAndPreprocess(
  ...
  } else if (useCifar10Normalization) {
    mean = cifar10NormMean;
  }  else {
    mean = zeroMean;
  }
