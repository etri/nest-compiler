...
void NestPartitionerSchedule::generateMain(std::string &wfilec, std::string &wfiled, std::string &wfilee,int partitionNum, std::vector<Function *> funcList, const PartitionConfig &partitionConfig) {
//  cout << "generateMain: " << wfilec << endl;

  std::cout << "generateMain os_partition_main_tyep = " << os_partition_main_type << std::endl;

  wfilee.append("START LAYER: 0  END LAYER: " + std::to_string(partitionNum-2) + "\n");
...
//    cout << "//======== inputName List Node " << i << " =======//" << endl;
  ...
  /*    if (node->getNumInputs() == 2) {
          wfilec.append("\t//node get number inputs " + std::to_string(node->getNumInputs()) + "\n");
...
//          cout << "node input KindName = " << node->getNthInput(count).getNode()->getKindName() << endl;
  ...
  //        std::cout << inputList[inputCount] << std::endl;
    ...
    if(i == 0)
    {
      wfilec.append("static void predicting_resnet18_layer" + std::to_string(i) + "(ManagerPredictingLayer *manager_layer){\n\n");
...
//          wfilec.append("\tchar *image = manager_layer->input_image;\n");
//         wfilec.append("\tinputImageFilenames.push_back(image);\n");
          wfiled.append("float *resultVar" + std::to_string(resultVarNum) + "_" + std::to_string(outputNum + 1) +";\n");
          wfilec.append("\tresultVar" + std::to_string(resultVarNum) + "_" + std::to_string(outputNum + 1) +
                        " = getInferenceResultVar(p" + std::to_string(i) +
                        "_config, mutableWeightVarsAddr" + std::to_string(i) +
      ...
      for(int count = 1; count < inputList->size(); count++) {
        ...
        if(flag) {
          wfilec.append("\tcopyMutableWeightVarsWithoutAlloc(p" + std::to_string(i) + "_config, mutableWeightVarsAddr" + std::to_string(i) +
            ...
            if( i != (partitionNum -2)){
               wfilee.append("PARTITION LAYER NUMBER:" + std::to_string(i) + "  RETURN VALUE:resultVar" + std::to_string(i) + "  OUTPUT FUNCTION:" + outputList[0] + ";\n");
                os_partition_flag = 1;
...
//        wfilec.append("\tprintf(\"\\n====> [Total inference time] %lu microsec.\\n\", inftime_total);\n\n");
        ...
        if(os_partition_main_type == 0) {
            ...
            wfilec.append("\tint resultLabel = dumpInferenceResults(p" + std::to_string(i) +
                          "_config, mutableWeightVarsAddr" + std::to_string(i) +
                          ", \"" + outputName + "\");\n\n");
        ...
        if(flag)  // 중복된 inputName이 없다면(전 파티션 outputName들 비교)
        {
          if(outputCount == 1)
          {
            ...
            if( os_partition_flag == 1) {
                wfilec.append("\tmanager_layer->returnVal = resultVar" + std::to_string(i) + ";\n");
                wfilec.append("\tmanager_layer->returnValSize = p" + std::to_string(i) + "_config.symbolTable[2].size * 4;\n\n");
//                wfilec.append("\tprintf(\" name = %s \\n\", p" + std::to_string(i) +"_config.symbolTable[2].name);\n");
//                wfilec.append("\tprintf(\" size = %d \\n\", manager_layer->returnValSize);\n");
                os_partition_flag = 0;
            }

//            wfilec.append("\tmanager_layer->freeVarAddrList[manager_layer->count_freeVarAddrList++] = mutableWeightVarsAddr" + std::to_string(i) + ";\n");
...
//set_input_layer_data
    wfilec.append("void set_input_layer_data(ManagerPredictingLayer *manager_layer){\n\n");

    for(int i = 1; i < partitionNum - 1; i++)
    {
        wfilec.append("\tif (manager_layer->start_layer == " + std::to_string(i) + ") {\n");
        wfilec.append("\tresultVar" + std::to_string(i-1) + " = manager_layer->input_layer_data;\n\n");
        wfilec.append("\t}\n\n");
    }
...
// prediction_layer_manager_create
    wfilec.append("ManagerPredictingLayer * prediction_layer_manager_create(char *input_image, float *input_layer_data, int start_layer, int end_layer) {\n\n");
    wfilec.append("\tManagerPredictingLayer *manager_layer;\n\n");
    wfilec.append("\tprintf(\"prediction_layer_create\\n\");\n");
    wfilec.append("\tmanager_layer = (ManagerPredictingLayer *) malloc(sizeof(ManagerPredictingLayer));\n\n");
...
//    wfilec.append("\tmanager_layer->count_freeVarAddrList =  0;\n\n");
    ...
    for(int i = 0; i < partitionNum - 1; i++) {
        wfilec.append("\tmanager_layer->predicting_layers[" + std::to_string(i) + "] = predicting_resnet18_layer" +
                      std::to_string(i) + ";\n");
    }
...
// prediction_layer_exit
    wfilec.append("void prediction_layer_exit(ManagerPredictingLayer *manager_layer) {\n\n");
    wfilec.append("\tprintf(\"prediction_layer_exit\\n\");\n\n");
    wfilec.append("\t//for (int i=0; i<manager_layer->count_freeVarAddrList; i++)\n");
...
// running_resnet18
    wfilec.append("void running_resnet18(char *input_image, float *input_layer_data, int start_layer, int end_layer) {\n\n");
    wfilec.append("\tint i;\n");
    wfilec.append("\tManagerPredictingLayer *manager_layer;\n\n");
    wfilec.append("\tmanager_layer = prediction_layer_manager_create(input_image, input_layer_data, start_layer, end_layer);\n\n");
    ...
    wfilec.append("\t}\n");
    wfilec.append("\telse {\n\n");
    wfilec.append("\t\tprintf(\"end layer size %d\\n\", manager_layer->returnValSize);\n");
    wfilec.append("\t\tresultVar = (float *)malloc(sizeof(float) * manager_layer->returnValSize);\n");
    wfilec.append("\t\tmemcpy(resultVar, manager_layer->returnVal, manager_layer->returnValSize);\n\n");
    wfilec.append("\t}\n\n");
...
//    for(int i = 0; i < partitionNum - 1; i++)
//    {
//        wfilec.append("\tpredicting_resnet18_layer" + std::to_string(i) + "(NULL);\n\n");
//    }

        wfilec.append("\trunning_resnet18(input_image, input_layer_data, 0," + std::to_string(partitionNum - 2) + ");\n");
...
void NestPartitionerSchedule::generateUnloadingResnet18(string& wfilec, int partitionNum, const PartitionConfig &partitionConfig) {
...
void NestPartitionerSchedule::generateLoadingResnet18(string& wfilec, int partitionNum, const PartitionConfig &partitionConfig) {
    ...
    for(int i = 1; i < partitionNum - 1; i++) {
        wfilec.append("\tmutableWeightVarsAddr" + std::to_string(i) + " =  allocateMutableWeightVars(p" +
...
void NestPartitionerSchedule::generateLayerManager(string& wfilec, int partitionNum, const PartitionConfig &partitionConfig) {
...
//                        "\tint count_freeVarAddrList = 0;\n\n");
    ...
    wfilec.append("\tvoid (*predicting_layers[" + std::to_string(partitionNum -1) + "])(struct manager_predicting_layer *);\n\n"
                        "} ManagerPredictingLayer;\n\n");

}


void NestPartitionerSchedule::generateParseLayerConfiguration(string& wfilec, int partitionNum, const PartitionConfig &partitionConfig) {
...
void NestPartitionerSchedule::generateGlobalVariables(string& wfilec, int partitionNum, const PartitionConfig &partitionConfig) {
...
//  std::cout << "out dir = " << outputDir_ << std::endl;
  ...
  string make_os_partition_layer_info_outfile_c = outputDir_ + "/os_partition_layer_info.txt";  //
  string make_os_partition_nnobject_outfile_c = outputDir_ + "/nn_object/nn_object_23.c";  //
...
//    string memfree;
    ...
    //generateParseLayerConfiguration(parseLayerConfiguration, partitionNum, partitionConfig);
    generateLoadingResnet18(loadingResnet, partitionNum, partitionConfig);

    generateMain(inference_main, globalVariable, osPartitionLayer, partitionNum, funcList, partitionConfig);
...
//    writeFileC << parseLayerConfiguration;
  ...
  if(writeMakeFileOsC.is_open()) {
     ...
     if( os_partition_main_type == 0) {
         generateMakeFileLocal(makeFileOs, partitionNum, funcList, partitionConfig);
     } else if ( os_partition_main_type == 1 ) {
         generateMakeFileNestOs(makeFileOs, partitionNum, funcList, partitionConfig);
     }
  ...
  //NEST-OS client code generation
    ...
    if ( os_partition_main_type == 1 ) {
        ...
        string make_os_client_resnet18_outfile_c = outputDir_ + "/client/resnet18.c";  //
...
void NestPartitionerSchedule::generateNnloadFile(string& wfilec, std::size_t partitionNum, std::vector<Function *> funcList, const PartitionConfig &partitionConfig) {
    ...
    wfilec.append("\t{\n");
    wfilec.append("\t\tloader->load_partition_range(loader, MODEL_PART_DYNAMIC_NAME, " + std::to_string(0) + ", " + std::to_string(partitionNum-2) + ", 0); // install into NPU OS #0\n");
    ...
    wfilec.append("\t\t{\n");
    wfilec.append("\t\t\tloader->unload_partition_range(loader, MODEL_PART_DYNAMIC_NAME, " + std::to_string(0) + ", " + std::to_string(partitionNum-2) + ", 0); // uninstall into NPU OS #0\n");
    wfilec.append("\t\t}\n");
    wfilec.append("\t}\n");
    wfilec.append("}\n");
...
void NestPartitionerSchedule::generateResnetFile(string& wfilec, std::size_t partitionNum, std::vector<Function *> funcList, const PartitionConfig &partitionConfig) {
    ...
    if( clientPartitionFlag == 1 ) {
        ...
        wfilec.append("{\n\n");
        wfilec.append("\tnt->get_data_output(nt, &output_offset2, &output_size2);\n");
    ...
    wfilec.append("\t/* print all the files and directories within directory */\n");
    ...
    if( clientPartitionFlag == 1 ) {
        ...
        wfilec.append("\t// ntask create and do setting if necessary (affinity or priority)\n");
        wfilec.append("\twsmem = malloc(64*1024*1024); // 64MB\n");
        wfilec.append(
                "\tNtask *nt1 = ntask_partition_range_create(\"nt1\", RESNET18_NPU_DYNAMIC, " + std::to_string(0) +
                ", " + std::to_string(clientPartitionNumber) +
                ", ntask_input_handler1, ntask_output_handler1, wsmem);\n");
        wfilec.append("\tNtask *nt2 = ntask_partition_range_create(\"nt2\", RESNET18_NPU_DYNAMIC, " +
                      std::to_string(clientPartitionNumber + 1) + ", " + std::to_string(partitionNum - 2) +
    ...
    if( clientPartitionFlag == 1 ) {
        ...
        wfilec.append("\t\tint* num = (int*)(nt2->get_memory_pointer(nt2)+output_offset2);\n");
    } else {

        wfilec.append("\t\tnt1->run(nt1);\n\n");
        wfilec.append("\t\tint* num = (int*)(nt1->get_memory_pointer(nt1)+output_offset);\n\n");
    }
    ...
    wfilec.append("int main(int argc, char *argv[])\n");
    wfilec.append("{\n");
    wfilec.append("\tfprintf(stderr, \"Demo needs OpenCV for mp4 images.\\n\");\n");
    wfilec.append("}\n");
    wfilec.append("#endif\n");

}

void NestPartitionerSchedule::generateNnobjectFile(string& wfilec, std::size_t partitionNum, std::vector<Function *> funcList, const PartitionConfig &partitionConfig) {
    ...
    wfilec.append("extern void save_image_options(Image im, const char *name, IMTYPE f, int quality);\n");
    wfilec.append("static char *image_path;\n\n");
    ...
    wfilec.append("\tint end_layer = info->part_num_start + info->part_num_delta;\n\n");
    wfilec.append("\tprintf(\" predict start %d end %d\\n\", start_layer, end_layer);\n\n");
    ...
    wfilec.append("\trunning_resnet18(image, input_layer_data, start_layer, end_layer);\n");
    wfilec.append("}\n\n");

    wfilec.append("static void set_input(NnObject *object, NetInfo *info, Input *input)\n");
    wfilec.append("{\n");
    wfilec.append("\tprintf(\"set_input start layer %d , end layer %d\\n\", info->part_num_start, info->part_num_start + info->part_num_delta);\n\n");
    wfilec.append("\tif (info->part_num_start  == 0) {\n");
    ...
    wfilec.append("static void get_output(NnObject *object, NetInfo *info, Output *output)\n");
    wfilec.append("{\n\n");
    wfilec.append("\tint end_layer = info->part_num_start + info->part_num_delta;\n\n");
    wfilec.append("\tif (end_layer == " + std::to_string(partitionNum-2) + ") {\n\n");
    ...
    wfilec.append("\t\tmemcpy(object->wsmem + output->data_offset, &resultLabel, output->data_size);\n");
    wfilec.append("\t}\n");
    wfilec.append("\telse {\n");
    wfilec.append("\t\toutput->data_offset = 0;\n");
    wfilec.append("\t\toutput->data_size = resultSize;\n");
    wfilec.append("\t\tmemcpy(object->wsmem + output->data_offset, resultVar, output->data_size);\n\n");
    wfilec.append("\tfree(resultVar);\n");
    wfilec.append("\t}\n\n");
    wfilec.append("}\n\n");

    wfilec.append("NnObject nn_object_23 = {23, 0, 0, {0, 0, 0, 0}, 0, load, unload, predict, set_input, get_output};\n");
...
void NestPartitionerSchedule::generateNnobjectFile(string& wfilec, std::size_t partitionNum, std::vector<Function *> funcList, const PartitionConfig &partitionConfig) {
    ...
    wfilec.append("#if 0\n");
    wfilec.append("#else\n");
    wfilec.append("\tint *dataptr = object->wsmem + (input->data_size - 8);\n\n");
    ...
    wfilec.append("NnObject nn_object_23 = {23, 0, 0, 0, load, unload, predict, set_input, get_output};\n");

}
*/


void NestPartitionerSchedule::generateMakeFileNestOs(string& wfilec, std::size_t partitionNum, std::vector<Function *> funcList, const PartitionConfig &partitionConfig) {
//  std::cout << "== [Start] generateCMakeListsFile == " << std::endl;
    ...
    wfilec.append("NOS_HOME=$(NEST_HOME)/os_dynamic_resnet18_npu_os_partition\n\n");
    ...
    wfilec.append("endif\n\n");
    wfilec.append("CFLAGS=-I. -I$(NOS_HOME)/include -I$(NEST_HOME)/include -Ios -Inn -Icpu -Idemo -g -std=c99 -Wall -Wno-unused-result -Wno-unknown-pragmas -Wfatal-errors -fPIC $(OPT)\n");
    wfilec.append("CFLAGS+=-D_GNU_SOURCE\n");
    wfilec.append("ifeq ($(NPU), 1)\n");
    wfilec.append("CFLAGS+=-Inpu\n");
    wfilec.append("CFLAGS+=-DUSENPU\n");
    wfilec.append("endif\n");
    wfilec.append("CPPFLAGS=-I. -Iinclude -I../include -Ios -Inn -Icpu -Idemo -g -Wall -Wno-unused-result -Wno-unknown-pragmas -Wfatal-errors -fPIC $(OPT)\n");
    ...
    wfilec.append("CPPFLAGS+= -I/root/nestc_vtabundle/vtalib/include/zcu102/ -I/root/nestc_vtabundle/vtalib/include/Bundle\n\n");
    ...
    wfilec.append("LDFLAGS+= -L/root/nestc_vtabundle/vtalib/zcu102 -Wl,-rpath,/root/nestc_vtabundle/vtalib/zcu102 -lVTABundle -lvta_runtime /usr/lib/llvm-8.0/lib/libLLVMSupport.a -lcma -lgomp -lpng -lz -lrt -ldl -ltinfo -lpthread -lm /usr/lib/llvm-8.0/lib/libLLVMDemangle.a\n\n");
    ...
    wfilec.append("\t$(CPP) $(OBJ_FILES1) $(OBJ_FILES) $(LIB_OBJ_FILES) -o $@ $(LDFLAGS)\n\n");
    ...
    wfilec.append("\trm -f $(OBJ_FILES) $(CPP_OBJ_FILES) $(CFG_OBJ_FILES) $(APP_OBJ_FILES) nn_object_2*.o\n");
    wfilec.append("\trm -f *.stackdump resenet_main libresenet18.a\n");
    wfilec.append("libmake:\n");
    wfilec.append("\tar rscv $(LIBNAME) $(OBJ_FILES) $(OBJ_FILES1) $(LIB_OBJ_FILES) $(AIWARE_OBJ_FILES)\n");
}

void NestPartitionerSchedule::generateMakeFileLocal(string& wfilec, std::size_t partitionNum, std::vector<Function *> funcList, const PartitionConfig &partitionConfig) {
//  std::cout << "== [Start] generateCMakeListsFile == " << std::endl;
