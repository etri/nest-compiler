...
class VTAInterpreterFunction final : public CompiledFunction,
...
public:
  VTAInterpreterFunction(std::unique_ptr<IRFunction> F,
                         runtime::RuntimeBundle &&bundle);

  /// \name CompiledFunction interface
  ///@{
  ~VTAInterpreterFunction() override;
  ...
  virtual std::string getCompileBackendName() const override {
    return "VTAInterpreter";
  }
  ///@}
};

/// An VTAInterpreterFunction bound to a specific invocation.
class BoundVTAInterpreterFunction : public IRInstructionProcessingHandler {
  ...
  /// A reference to the constant map from the owning VTAInterpreterFunction.
  const std::unordered_map<std::string, Tensor *> &constants_;

public:
  explicit BoundVTAInterpreterFunction(
      const std::unordered_map<std::string, Tensor *> &constants)
      : constants_(constants) {}

  ~BoundVTAInterpreterFunction();
