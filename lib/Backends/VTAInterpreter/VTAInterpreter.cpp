...
#include "glow/Backends/VTAInterpreter/VTAInterpreterFunction.h"
...
namespace glow {
namespace runtime {
extern unsigned GlowVTAInterpreterMemory;
}
} // namespace glow
using namespace glow;

Expected<std::unique_ptr<CompiledFunction>>
VTAInterpreter::compile(Function *F, const BackendOptions &opts) const {
...
VTAInterpreter::compileIR(std::unique_ptr<IRFunction> IR) const {
  auto *mod = IR->getGraph()->getParent();
  auto function = compileIRWithoutConstants(std::move(IR));
  auto IFunction = static_cast<VTAInterpreterFunction *>(function.get());
  IFunction->collectConstants(mod);
  return function;
}

std::unique_ptr<CompiledFunction> VTAInterpreter::compileIRWithoutConstants(
  ...
  auto compiledFunction = glow::make_unique<VTAInterpreterFunction>(
...
static bool checkLayout(const T &I) {
  if (I.getLayout() != NHWC && I.getLayout() != NTHWC) {
    report("Glow VTAInterpreter supports only NHWC and NTHWC");
    return false;
  }
  return true;
}
...
Expected<bool> VTAInterpreter::transformPostLowering(
    Function *F, CompilationContext &cctx,
    const glow::runtime::DeviceInfo *devInfo) const {
  LOG_SCOPE(F->getLogContext(), "VTAInterpreter::transformPostLowering")
...
void VTAInterpreter::parseBackendSpecificOptions(
    const BackendOptions &opts) const {
  auto VTAInterpreterMaxMemOpt =
      opts.backendSpecificOpts.find("VTAInterpreter-memory");
  if (VTAInterpreterMaxMemOpt != opts.backendSpecificOpts.end()) {
    glow::runtime::GlowVTAInterpreterMemory =
        std::stoi(VTAInterpreterMaxMemOpt->second);
    llvm::outs() << "VTAInterpreter memory set to "
                 << glow::runtime::GlowVTAInterpreterMemory << "\n";
  }
}
