...
#define dispatchSplitEmbeddingTypeImpl(functionName, indexTy, outputTy,        \
                                       weightsOffsetTy, ...)                   \
  switch (indexTy) {                                                           \
  case ElemKind::Int32ITy:                                                     \
    if (outputTy == SplitEmbeddingSparseType::EST_FLOAT) {                     \
      if (weightsOffsetTy == ElemKind::Int32ITy) {                             \
        functionName<int32_t, float, int32_t>(__VA_ARGS__);                    \
      } else if (weightsOffsetTy == ElemKind::Int64ITy) {                      \
        functionName<int32_t, float, int64_t>(__VA_ARGS__);                    \
      }                                                                        \
    } else if (outputTy == SplitEmbeddingSparseType::EST_FLOAT16) {            \
      if (weightsOffsetTy == ElemKind::Int32ITy) {                             \
        functionName<int32_t, float16, int32_t>(__VA_ARGS__);                  \
      } else if (weightsOffsetTy == ElemKind::Int64ITy) {                      \
        functionName<int32_t, float16, int64_t>(__VA_ARGS__);                  \
      }                                                                        \
    }                                                                          \
    break;                                                                     \
  case ElemKind::Int64ITy:                                                     \
    if (outputTy == SplitEmbeddingSparseType::EST_FLOAT) {                     \
      if (weightsOffsetTy == ElemKind::Int32ITy) {                             \
        functionName<int64_t, float, int32_t>(__VA_ARGS__);                    \
      } else if (weightsOffsetTy == ElemKind::Int64ITy) {                      \
        functionName<int64_t, float, int64_t>(__VA_ARGS__);                    \
      }                                                                        \
    } else if (outputTy == SplitEmbeddingSparseType::EST_FLOAT16) {            \
      if (weightsOffsetTy == ElemKind::Int32ITy) {                             \
        functionName<int64_t, float16, int32_t>(__VA_ARGS__);                  \
      } else if (weightsOffsetTy == ElemKind::Int64ITy) {                      \
        functionName<int64_t, float16, int64_t>(__VA_ARGS__);                  \
      }                                                                        \
    }                                                                          \
    break;                                                                     \
  default:                                                                     \
...
void BoundVTAInterpreterFunction::fwdConvolutionInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdConvolutionInstQuantizedImpl(
    ...
    llvm::ArrayRef<unsigned_t> kernelSizes, llvm::ArrayRef<unsigned_t> strides,
    llvm::ArrayRef<unsigned_t> pads, size_t group,
    llvm::ArrayRef<unsigned_t> dilation, const int32_t fuseRelu) {
  auto inW = getWeightHandle<ElemTy>(inV);
  auto outW = getWeightHandle<ElemTy>(outV);
  auto filterW = getWeightHandle<ElemTy>(filterV);
      ...
      for (dim_t d = g * outCperG; d < (g + 1) * outCperG; d++) {
        ...
        for (dim_t ax = 0; ax < odim.h; x += sdim.height, ax++) {
          ...
          for (dim_t ay = 0; ay < odim.w; y += sdim.width, ay++) {
            ...
            /*
            AccumulatorTy B;
            float bTemp = float(biasW.at({d}) - biasOffset) * (biasScale /
            matMulScale); if(bTemp>0){ B = std::floor(bTemp); } else{ B =
            std::ceil(bTemp);
            }
*/
            // Add the bias.
            ...
            /*
            float outTemp = float(sum) * (matMulScale / outScale) + outOffset;
            if(outTemp>0){
              outW.at({n, ax, ay, d}) = quantization::clip<AccumulatorTy,
            ElemTy>(std::floor(outTemp)); } else{ outW.at({n, ax, ay, d}) =
            quantization::clip<AccumulatorTy, ElemTy>(std::ceil(outTemp));
            }
             */
            if (fuseRelu) {
              outW.at({n, ax, ay, d}) =
...
void BoundVTAInterpreterFunction::fwdVTAInterpreterConvolutionInst(
  ...
  cpuvtaconvolution(in_NC_HW16, 1.0, 0, filter_NC_HW16, 1.0, 0, bias_NC_HW16,
                    ...
                    fdim.w, pad_size, stride_size, 1, {1, 1}, doRelu, doBias,
                    odim.h, odim.w);
...
int BoundVTAInterpreterFunction::cpuvtaconvolution(
    ...
    int pad_size, int stride_size, size_t group,
    llvm::ArrayRef<unsigned_t> dilation, bool doRelu, bool doBias, int out_h,
    int out_w) {
          ...
          for (dim_t ay = 0; ay < out_w; y += stride_size, ay++) {
            ...
            for (dim_t fx = 0; fx < KH; fx++) {
              for (dim_t fy = 0; fy < KW; fy++) {
                int64_t ox = x + fx * dilation[0];
                int64_t oy = y + fy * dilation[1];
...
void BoundVTAInterpreterFunction::fwdConvTransposeInst(
...
void BoundVTAInterpreterFunction::fwdConvolutionInst(const ConvolutionInst *I) {
  auto kernelSizes = I->getKernels();
  auto pads = I->getPads();
  auto strides = I->getStrides();
  size_t group = I->getGroup();
...
void BoundVTAInterpreterFunction::fwdFusionGroupInst(
    const glow::FusionGroupInst *p) {
  llvm_unreachable(
      "Fusion Group instruction not supported on Inrerpretter backend\n");
}

void BoundVTAInterpreterFunction::fwdConcatInst(const ConcatInst *I) {
  (void)I;
  // TODO
  llvm_unreachable("not yet implemented");
}

void BoundVTAInterpreterFunction::fwdConvolutionGradInst(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdConvolution3DInstFloatImpl(
    Value *inV, Value *outV, Value *filterV, Value *biasV,
    llvm::ArrayRef<unsigned_t> kernelSizes, llvm::ArrayRef<unsigned_t> strides,
    llvm::ArrayRef<unsigned_t> pads, size_t group) {
  staticAssertFloatingPointType(ElemTy);
...
template <typename ElemTy, typename AccumulatorTy, typename BiasElemTy>
void BoundVTAInterpreterFunction::fwdConvolution3DInstQuantizedImpl(
    Value *inV, Value *outV, Value *filterV, Value *biasV,
    llvm::ArrayRef<unsigned_t> kernelSizes, llvm::ArrayRef<unsigned_t> strides,
    llvm::ArrayRef<unsigned_t> pads, size_t group) {
  auto inW = getWeightHandle<ElemTy>(inV);
...
void BoundVTAInterpreterFunction::fwdConvolution3DInst(
...
void BoundVTAInterpreterFunction::fwdConvolution3DGradInst(
...
//===----------------------------------------------------------------------===//
template <typename ElemTy, typename AccumulatorTy, typename BiasElemTy>
void BoundVTAInterpreterFunction::fwdChannelwiseQuantizedConv2DInstImpl(
...
template <typename ElemTy, typename AccumulatorTy, typename BiasElemTy>
void BoundVTAInterpreterFunction::fwdChannelwiseQuantizedConv3DInstImpl(
    const ChannelwiseQuantizedConvolutionInst *I) {
  auto inW = getWeightHandle<ElemTy>(I->getSrc());
  auto outW = getWeightHandle<ElemTy>(I->getDest());
  auto filterW = getWeightHandle<ElemTy>(I->getFilter());
...
void BoundVTAInterpreterFunction::fwdChannelwiseQuantizedConvolutionInst(
    const ChannelwiseQuantizedConvolutionInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdBatchNormalizationFloatImpl(
...
void BoundVTAInterpreterFunction::fwdBatchNormalizationI8Impl(
    const BatchNormalizationInst *I, int numDims) {
...
void BoundVTAInterpreterFunction::fwdBatchNormalizationInst(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdLayerNormalizationInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdLayerNormalizationInst(
...
void BoundVTAInterpreterFunction::fwdMaxPoolInst(const MaxPoolInst *I) {
...
void BoundVTAInterpreterFunction::fwdMaxPoolWithArgmaxInst(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdAvgPoolInstFloatImpl(
    const AvgPoolInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdAvgPoolInstI8Impl(const AvgPoolInst *I) {
  ShapeNHWC odim(I->getDest()->dims());
  ShapeNHWC idim(I->getSrc()->dims());
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdAvgPool3DInstFloatImpl(
    const AvgPoolInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdAvgPool3DInstI8Impl(const AvgPoolInst *I) {
  ShapeNTHWC odim(I->getDest()->dims());
  ShapeNTHWC idim(I->getSrc()->dims());
...
void BoundVTAInterpreterFunction::fwdAvgPoolInst(const AvgPoolInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdAdaptiveAvgPoolInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdAdaptiveAvgPoolInstI8Impl(
    const AdaptiveAvgPoolInst *I) {
  ShapeNHWC odim(I->getDest()->dims());
  ShapeNHWC idim(I->getSrc()->dims());
...
void BoundVTAInterpreterFunction::fwdAdaptiveAvgPoolInst(
    const AdaptiveAvgPoolInst *I) {
  if (I->getSrc()->getType()->isQuantizedType()) {
...
void BoundVTAInterpreterFunction::fwdAdaptiveAvgPoolGradInst(
...
void BoundVTAInterpreterFunction::fwdAvgPool2DGradInst(
    const AvgPoolGradInst *I) {
  auto inG = getWeightHandle(I->getSrcGrad());
  auto outW = getWeightHandle(I->getDest());
  auto outG = getWeightHandle(I->getDestGrad());
...
void BoundVTAInterpreterFunction::fwdAvgPool3DGradInst(
    const AvgPoolGradInst *I) {
  auto inG = getWeightHandle(I->getSrcGrad());
  auto outW = getWeightHandle(I->getDest());
  auto outG = getWeightHandle(I->getDestGrad());
...
void BoundVTAInterpreterFunction::fwdAvgPoolGradInst(const AvgPoolGradInst *I) {
  bool isConv3D = is3DData(ConvolutionLayout(I->getLayout()));
...
void BoundVTAInterpreterFunction::fwdReluInst(const ReluInst *I) {
  ...
  if (I->getSrc()->getType()->isQuantizedType()) {
    ...
    */
    ...
    dispatchFloatingPointImpl(fwdReluInstFloatImpl,
                              I->getSrc()->getElementType(), I);
  }
}
...
void BoundVTAInterpreterFunction::fwdVTAReluInstQuantizedImpl(Value *inV,
                                                              Value *outV,
                                                              float scale) {
  auto inW = getWeightHandle<int8_t>(inV);
  auto outW = getWeightHandle<int8_t>(outV);
...
void BoundVTAInterpreterFunction::fwdClipInst(const ClipInst *) {
  DCHECK(!"Found ClipInst but Clip is lowered on VTAInterpreter");
}

void BoundVTAInterpreterFunction::fwdLeakyReluInst(const LeakyReluInst *) {
  DCHECK(!"Found LeakyReluInst but LeakyRelu is lowered on VTAInterpreter");
}

void BoundVTAInterpreterFunction::fwdHardSwishInst(const HardSwishInst *) {
  DCHECK(!"Found HardSwishInst but HardSwish is lowered on VTAInterpreter");
}

template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdSigmoidInstFloatImpl(
    const SigmoidInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdSigmoidInst(const SigmoidInst *I) {
  dispatchFloatingPointImpl(fwdSigmoidInstFloatImpl,
                            I->getSrc()->getElementType(), I);
}

template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdTanhInstFloatImpl(const TanhInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdTanhInst(const TanhInst *I) {
  dispatchFloatingPointImpl(fwdTanhInstFloatImpl, I->getSrc()->getElementType(),
                            I);
}

template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdSoftPlusInstFloatImpl(
    const SoftPlusInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdSoftPlusInst(const SoftPlusInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdSoftMaxInstImpl(const SoftMaxInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdSoftMaxInst(const SoftMaxInst *I) {
  dispatchFloatingPointImpl(fwdSoftMaxInstImpl, I->getSrc()->getElementType(),
                            I);
}

void BoundVTAInterpreterFunction::fwdSoftMaxGradInst(const SoftMaxGradInst *I) {
  auto inG = getWeightHandle(I->getSrcGrad());
  auto idim = inG.dims();
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdLogSoftMaxInstImpl(
    const LogSoftMaxInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdLogSoftMaxInst(const LogSoftMaxInst *I) {
  dispatchFloatingPointImpl(fwdLogSoftMaxInstImpl,
                            I->getSrc()->getElementType(), I);
}

template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdCrossEntropyLossInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdCrossEntropyLossInst(
    const CrossEntropyLossInst *I) {
  dispatchFloatingPointImpl(fwdCrossEntropyLossInstFloatImpl,
                            I->getP()->getElementType(), I);
}

void BoundVTAInterpreterFunction::fwdCrossEntropyLossGradInst(
...
void BoundVTAInterpreterFunction::fwdCopyInst(const CopyInst *I) {
  auto inT = getTensor(I->getSrc());
  auto outT = getTensor(I->getDest());
  outT->copyRawFrom(inT);
}

void BoundVTAInterpreterFunction::fwdTransposeInst(const TransposeInst *I) {
  auto inT = getTensor(I->getSrc());
  (void)inT;
  auto outT = getTensor(I->getDest());
...
void BoundVTAInterpreterFunction::fwdTensorViewInst(const TensorViewInst *I) {
  getOrCreateUnownedTensor(I, I->getSrc(), I->getOffsets());
}

void BoundVTAInterpreterFunction::fwdSplatInst(const glow::SplatInst *I) {
...
void BoundVTAInterpreterFunction::fwdTouchInst(const glow::TouchInst *) {
  // Do nothing for a TouchInst
}

void BoundVTAInterpreterFunction::fwdInsertTensorInst(
...
void BoundVTAInterpreterFunction::fwdExtractTensorInst(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdGatherInstImpl(const glow::GatherInst *I) {
...
void BoundVTAInterpreterFunction::fwdGatherInst(const glow::GatherInst *I) {
...
void BoundVTAInterpreterFunction::fwdGatherElementsInstImpl(
...
void BoundVTAInterpreterFunction::fwdGatherElementsInst(
    const glow::GatherElementsInst *I) {
  switch (I->getIndices()->getElementType()) {
  case ElemKind::Int64ITy:
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdGatherNDInstImpl(
...
void BoundVTAInterpreterFunction::fwdGatherNDInst(const glow::GatherNDInst *I) {
  switch (I->getIndices()->getElementType()) {
  case ElemKind::Int64ITy:
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdGatherRangesInstImpl(
...
void BoundVTAInterpreterFunction::fwdGatherRangesInst(
    const glow::GatherRangesInst *I) {
...
void BoundVTAInterpreterFunction::fwdScatterDataInstCopyImpl(
...
template <typename ElemTy, typename IndicesElemTy>
void BoundVTAInterpreterFunction::fwdScatterDataInstAddFloatImpl(
    const glow::ScatterDataInst *I) {
  Tensor *dataT = getTensor(I->getData());
  Tensor *indicesT = getTensor(I->getIndices());
  Tensor *slicesT = getTensor(I->getSlices());
...
template <typename ElemTy, typename IndicesElemTy>
void BoundVTAInterpreterFunction::fwdScatterDataInstAddQuantizedImpl(
    const glow::ScatterDataInst *I) {
  Tensor *dataT = getTensor(I->getData());
  Tensor *indicesT = getTensor(I->getIndices());
  Tensor *slicesT = getTensor(I->getSlices());
...
void BoundVTAInterpreterFunction::fwdScatterDataInst(
    const glow::ScatterDataInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdBatchOneHotImpl(
...
void BoundVTAInterpreterFunction::fwdBatchOneHotInst(
    const glow::BatchOneHotInst *I) {
  switch (I->getData()->getElementType()) {
  case ElemKind::Int64ITy:
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdSpaceToDepthInstImpl(
...
void BoundVTAInterpreterFunction::fwdSpaceToDepthInst(
    const glow::SpaceToDepthInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdResizeNearestInstImpl(
...
void BoundVTAInterpreterFunction::fwdResizeNearestInst(
    const ResizeNearestInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdResizeBilinearInstImpl(
...
void BoundVTAInterpreterFunction::fwdResizeBilinearInst(
    const ResizeBilinearInst *I) {
  if (getTensor(I->getSrc())->getType().isQuantizedType()) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdLocalResponseNormalizationInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdLocalResponseNormalizationInst(
    const LocalResponseNormalizationInst *I) {
  dispatchFloatingPointImpl(fwdLocalResponseNormalizationInstFloatImpl,
                            I->getSrc()->getElementType(), I);
}

void BoundVTAInterpreterFunction::fwdLocalResponseNormalizationGradInst(
...
void BoundVTAInterpreterFunction::fwdBucketizeInst(const BucketizeInst *I) {
...
//===----------------------------------------------------------------------===//
void BoundVTAInterpreterFunction::fwdElementAddInstI8Impl(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementAddInstArithmeticImpl(
    const ElementAddInst *I) {
...
void BoundVTAInterpreterFunction::fwdElementAddInst(const ElementAddInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementSubInstArithmeticImpl(
...
void BoundVTAInterpreterFunction::fwdElementSubInst(const ElementSubInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
    auto destTy = I->getDest()->getType();
    auto lhsTy = I->getLHS()->getType();
    auto rhsTy = I->getRHS()->getType();
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementMulInstArithmeticImpl(
...
void BoundVTAInterpreterFunction::fwdElementMulInst(const ElementMulInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
    auto lhsTy = I->getLHS()->getType();
    auto rhsTy = I->getRHS()->getType();
    auto destTy = I->getDest()->getType();
...
void BoundVTAInterpreterFunction::fwdElementFmodInst(const ElementFmodInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
    auto destTy = I->getDest()->getType();
    auto lhsTy = I->getLHS()->getType();
    auto rhsTy = I->getRHS()->getType();
...
void BoundVTAInterpreterFunction::fwdElementDivInst(const ElementDivInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
    auto destTy = I->getDest()->getType();
    auto lhsTy = I->getLHS()->getType();
    auto rhsTy = I->getRHS()->getType();
...
void BoundVTAInterpreterFunction::fwdElementMaxInstI8Impl(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementMaxInstArithmeticImpl(
    const ElementMaxInst *I) {
  staticAssertArithmeticType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdElementMaxInst(const ElementMaxInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementMinInstArithmeticImpl(
...
void BoundVTAInterpreterFunction::fwdElementMinInst(const ElementMinInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
    auto lhsTy = I->getLHS()->getType();
    auto rhsTy = I->getRHS()->getType();
    auto destTy = I->getDest()->getType();
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementBitwiseOrInstImpl(
...
void BoundVTAInterpreterFunction::fwdElementBitwiseOrInst(
    const ElementBitwiseOrInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementBitwiseAndInstImpl(
...
void BoundVTAInterpreterFunction::fwdElementBitwiseAndInst(
    const ElementBitwiseAndInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementBitwiseXorInstImpl(
...
void BoundVTAInterpreterFunction::fwdElementBitwiseXorInst(
    const ElementBitwiseXorInst *I) {
...
//                              Logical operations
//===----------------------------------------------------------------------===//
void BoundVTAInterpreterFunction::fwdElementNotInst(const ElementNotInst *I) {
...
void BoundVTAInterpreterFunction::fwdElementAndInst(const ElementAndInst *I) {
...
void BoundVTAInterpreterFunction::fwdElementOrInst(const ElementOrInst *I) {
  auto lhsW = getWeightHandle<bool>(I->getLHS());
  auto rhsW = getWeightHandle<bool>(I->getRHS());
  auto outW = getWeightHandle<bool>(I->getDest());
  for (size_t i = 0, e = outW.size(); i < e; ++i) {
...
void BoundVTAInterpreterFunction::fwdElementXorInst(const ElementXorInst *I) {
  auto lhsW = getWeightHandle<bool>(I->getLHS());
  auto rhsW = getWeightHandle<bool>(I->getRHS());
  auto outW = getWeightHandle<bool>(I->getDest());
  for (size_t i = 0, e = outW.size(); i < e; ++i) {
...
void BoundVTAInterpreterFunction::fwdUnaryArithmeticImpl(
...
void BoundVTAInterpreterFunction::fwdElementBitwiseNotInst(
...
void BoundVTAInterpreterFunction::fwdElementAbsInst(const ElementAbsInst *I) {
  auto func = [](float x) -> float { return std::abs(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAInterpreterFunction::fwdElementNegInst(const ElementNegInst *I) {
  auto func = [](float x) -> float { return -x; };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAInterpreterFunction::fwdElementFloorInst(
    const ElementFloorInst *I) {
...
void BoundVTAInterpreterFunction::fwdElementTruncateInst(
    const ElementTruncateInst *I) {
  auto func = [](float x) -> float { return std::trunc(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAInterpreterFunction::fwdElementRoundInst(
    const ElementRoundInst *I) {
...
void BoundVTAInterpreterFunction::fwdElementSqrtInst(const ElementSqrtInst *I) {
  auto func = [](float x) -> float { return std::sqrt(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAInterpreterFunction::fwdElementRsqrtInst(
    const ElementRsqrtInst *I) {
  auto func = [](float x) -> float { return 1 / std::sqrt(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAInterpreterFunction::fwdElementReciprocalInst(
    const ElementReciprocalInst *I) {
  auto func = [](float x) -> float { return 1 / x; };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAInterpreterFunction::fwdElementSinInst(const ElementSinInst *I) {
  auto func = [](float x) -> float { return std::sin(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAInterpreterFunction::fwdElementCosInst(const ElementCosInst *I) {
  auto func = [](float x) -> float { return std::cos(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAInterpreterFunction::fwdElementErfInst(const ElementErfInst *I) {
...
void BoundVTAInterpreterFunction::fwdElementCmpHelperImpl(
...
template <typename ElemTy, typename ElemOffsetTy, typename ElemScaleTy,
          typename CmpTy>
void BoundVTAInterpreterFunction::fwdElementCmpLTEInstImpl(
...
void BoundVTAInterpreterFunction::fwdElementCmpLTEInst(
    const ElementCmpLTEInst *I) {
...
template <typename ElemTy, typename ElemOffsetTy, typename ElemScaleTy,
          typename CmpTy>
void BoundVTAInterpreterFunction::fwdElementCmpEQInstImpl(
...
void BoundVTAInterpreterFunction::fwdElementCmpEQInst(
    const ElementCmpEQInst *I) {
  auto *T = getTensor(I->getLHS());
...
template <typename ElemTy, typename ElemOffsetTy, typename ElemScaleTy,
          typename CmpTy>
void BoundVTAInterpreterFunction::fwdElementCmpNEQInstImpl(
...
void BoundVTAInterpreterFunction::fwdElementCmpNEQInst(
    const ElementCmpNEQInst *I) {
  auto *T = getTensor(I->getLHS());
...
template <typename ElemTy, typename ElemOffsetTy, typename ElemScaleTy,
          typename CmpTy>
void BoundVTAInterpreterFunction::fwdElementCmpLTInstImpl(
...
void BoundVTAInterpreterFunction::fwdElementCmpLTInst(
    ElementCmpLTInst const *I) {
  auto *T = getTensor(I->getLHS());
  if (T->getType().isQuantizedType()) {
    switch (T->getElementType()) {
    case ElemKind::Int8QTy:
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementPowInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdElementPowInstI8Impl(
    const ElementPowInst *I) {
  assert(getTensor(I->getLHS())->getType().isQuantizedType() &&
...
void BoundVTAInterpreterFunction::fwdElementPowInst(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementIsNaNInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdElementIsNaNInst(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementLogInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdElementLogInst(const ElementLogInst *I) {
  dispatchFloatingPointImpl(fwdElementLogInstFloatImpl,
                            I->getSrc()->getElementType(), I);
}

template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementExpInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdElementExpInst(const ElementExpInst *I) {
  dispatchFloatingPointImpl(fwdElementExpInstFloatImpl,
                            I->getSrc()->getElementType(), I);
}

void BoundVTAInterpreterFunction::fwdNonZeroInst(const NonZeroInst *I) {
  auto *T = getTensor(I->getDest());
  T->zero();
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdElementSelectInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdElementSelectInst(
    const glow::ElementSelectInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
    auto destTy = I->getDest()->getType();
    auto lhsTy = I->getLHS()->getType();
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdModuloInstImpl(glow::ModuloInst const *I) {
...
void BoundVTAInterpreterFunction::fwdModuloInst(glow::ModuloInst const *I) {
...
void BoundVTAInterpreterFunction::fwdElementAcosInst(const ElementAcosInst *I) {
  auto func = [](float x) -> float { return std::acos(x); };
  dispatchImpl(fwdUnaryTrigonometricImpl, I->getSrc()->getElementType(), I,
               func);
}

void BoundVTAInterpreterFunction::fwdElementAsinInst(const ElementAsinInst *I) {
  auto func = [](float x) -> float { return std::asin(x); };
  dispatchImpl(fwdUnaryTrigonometricImpl, I->getSrc()->getElementType(), I,
               func);
}

void BoundVTAInterpreterFunction::fwdElementAtanInst(const ElementAtanInst *I) {
...
void BoundVTAInterpreterFunction::fwdMatMulInstQuantizedImpl(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdMatMulInstFloatImpl(const MatMulInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdBatchMatMulInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdMatMulInst(const glow::MatMulInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
...
void BoundVTAInterpreterFunction::fwdBatchMatMulInst(
...
void BoundVTAInterpreterFunction::fwdReluGradInst(const glow::ReluGradInst *I) {
...
//===----------------------------------------------------------------------===//
template <typename ElemTy, typename AccumulatorTy, typename BiasElemTy>
void BoundVTAInterpreterFunction::fwdFullyConnectedInstQuantizedImpl(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdFullyConnectedInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdFullyConnectedInst(
    const glow::FullyConnectedInst *I) {
...
void BoundVTAInterpreterFunction::fwdDynRowwiseQuantizedFullyConnectedInstImpl(
...
void BoundVTAInterpreterFunction::
...
void BoundVTAInterpreterFunction::fwdDynamicRowwiseQuantizedFullyConnectedInst(
...
void BoundVTAInterpreterFunction::fwdDynamicQuantizedFullyConnectedInst(
...
//===----------------------------------------------------------------------===//
template <typename ElemTy, typename AccumulatorTy, typename BiasElemTy>
void BoundVTAInterpreterFunction::fwdRowwiseQuantizedFullyConnectedInstImpl(
...
void BoundVTAInterpreterFunction::fwdRowwiseQuantizedFullyConnectedInst(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdBatchedAddInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdBatchedAddInst(
    const glow::BatchedAddInst *I) {
...
void BoundVTAInterpreterFunction::fwdBatchedReduceAddInst(
...
void BoundVTAInterpreterFunction::fwdBatchedReduceProdInst(
...
void BoundVTAInterpreterFunction::fwdCumSumInst(glow::CumSumInst const *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdLengthsSumInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdLengthsSumInst(const LengthsSumInst *I) {
  dispatchFloatingPointImpl(fwdLengthsSumInstFloatImpl,
                            I->getData()->getElementType(), I)
}

template <typename TI>
void BoundVTAInterpreterFunction::fwdSparseLengthsSumInstI8Impl(
...
void BoundVTAInterpreterFunction::fwdSparseLengthsSumInstFloatImpl(
    const SparseLengthsSumInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdSparseLengthsSumInst(
    const SparseLengthsSumInst *I) {
...
template <typename ElemTy, typename TI>
void BoundVTAInterpreterFunction::fwdSparseLengthsWeightedSumInstFloatImpl(
...
template <typename TI>
void BoundVTAInterpreterFunction::fwdSparseLengthsWeightedSumInstI8Impl(
    const SparseLengthsWeightedSumInst *I) {
...
void BoundVTAInterpreterFunction::fwdSparseLengthsSumGradInst(
    const SparseLengthsSumGradInst * /*I*/) {
  DCHECK(!"Found SparseLengthsSumGradInst but SparseLengthsSum is lowered on "
          "Interpreter");
}

void BoundVTAInterpreterFunction::fwdSparseLengthsWeightedSumInst(
    const SparseLengthsWeightedSumInst *I) {
  if (I->getDest()->getType()->isQuantizedType()) {
...
void BoundVTAInterpreterFunction::fwdSparseLengthsWeightedSumGradInst(
...
void BoundVTAInterpreterFunction::fwdEmbeddingBagInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdEmbeddingBagInst(
    const EmbeddingBagInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdEmbeddingInstImpl(
    Tensor *wtT, Tensor *indT, Tensor *outT, int64_t padIdx, bool sparse,
    bool scale, dim_t embedding_dim) {
...
void BoundVTAInterpreterFunction::fwdEmbeddingInst(const EmbeddingInst *I) {
...
void BoundVTAInterpreterFunction::
    fwdRowwiseQuantizedSparseLengthsWeightedSumImpl(
...
void BoundVTAInterpreterFunction::
    fwdRowwiseQuantizedSparseLengthsWeightedSumInst(
        const RowwiseQuantizedSparseLengthsWeightedSumInst *I) {
...
void BoundVTAInterpreterFunction::fwdFusedRowwiseQuantizedSparseLengthsSumInst(
    const FusedRowwiseQuantizedSparseLengthsSumInst *I) {
  llvm_unreachable("Not supported");
}

template <typename T, typename AccumT, typename IndexT>
void BoundVTAInterpreterFunction::fwdEmbeddingBagByteRowwiseOffsetsImpl(
...
void BoundVTAInterpreterFunction::fwdEmbeddingBagByteRowwiseOffsetsInst(
    const EmbeddingBagByteRowwiseOffsetsInst *I) {
  const auto ity = I->getIndices()->getElementType();
  const bool fp32FusedScaleOffset =
      (I->getData()->getElementType() == ElemKind::UInt4FusedQTy) ||
...
void BoundVTAInterpreterFunction::fwdLengthsToRangesInst(
...
void BoundVTAInterpreterFunction::fwdLengthsRangeFillInst(
...
void BoundVTAInterpreterFunction::fwdGaussianFillInst(
    const GaussianFillInst *I) {
...
void BoundVTAInterpreterFunction::fwdBatchSparseToDenseInstImpl2(
...
void BoundVTAInterpreterFunction::fwdBatchSparseToDenseInstImpl1(
    const BatchSparseToDenseInst *I) {
...
void BoundVTAInterpreterFunction::fwdBatchSparseToDenseInst(
    const BatchSparseToDenseInst *I) {
...
void BoundVTAInterpreterFunction::fwdFillExamplesWithIndicatorInstImpl2(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdFillExamplesWithIndicatorInstImpl1(
    const FillExamplesWithIndicatorInst *I) {
...
void BoundVTAInterpreterFunction::fwdFillExamplesWithIndicatorInst(
    const FillExamplesWithIndicatorInst *I) {
  dispatchArithmeticImpl(fwdFillExamplesWithIndicatorInstImpl1,
                         I->getDest()->getElementType(), I);
}
void BoundVTAInterpreterFunction::fwdSparseToDenseMaskInst(
...
void BoundVTAInterpreterFunction::fwdSparseLabelSplitInst(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdCollectRpnProposalsInstImpl(
...
void BoundVTAInterpreterFunction::fwdCollectRpnProposalsInst(
    const glow::CollectRpnProposalsInst *I) {
  dispatchFloatingPointImpl(fwdCollectRpnProposalsInstImpl,
                            I->getOperand(1).first->getElementType(), I);
}

void BoundVTAInterpreterFunction::fwdTopKInst(const TopKInst *I) {
...
void BoundVTAInterpreterFunction::fwdBatchedUnaryEmbeddingsBagsInst(
...
template <typename ElemTy, typename IndexType>
void BoundVTAInterpreterFunction::fwdBatchedUnaryEmbeddingsBagsInstImpl(
    const BatchedUnaryEmbeddingsBagsInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdIntNBitSplitEmbeddingBagsInst(
    const IntNBitSplitEmbeddingBagsInst *I) {
  dispatchSplitEmbeddingTypeImpl(
      fwdIntNBitSplitEmbeddingBagsInstImpl, I->getIndices()->getElementType(),
      I->getOutputDType(), I->getWeightsOffsets()->getElementType(), I);
}

void BoundVTAInterpreterFunction::fwdIntNBitSplitEmbeddingWeightedBagsInst(
    const IntNBitSplitEmbeddingWeightedBagsInst *I) {
  dispatchSplitEmbeddingTypeImpl(fwdIntNBitSplitEmbeddingWeightedBagsInstImpl,
                                 I->getIndices()->getElementType(),
                                 I->getOutputDType(),
                                 I->getWeightsOffsets()->getElementType(), I);
}

template <typename IndexTy, typename OutputTy, typename WeightsOffsetTy>
void BoundVTAInterpreterFunction::fwdIntNBitSplitEmbeddingBagsInstImpl(
    const IntNBitSplitEmbeddingBagsInst *I) {
  auto out = getTensor(I->getDest());
  ...
  fwdIntNBitSplitEmbeddingWeightedBagsImpl<IndexTy, OutputTy, WeightsOffsetTy>(
...
template <typename IndexTy, typename OutputTy, typename WeightsOffsetTy>
void BoundVTAInterpreterFunction::fwdIntNBitSplitEmbeddingWeightedBagsInstImpl(
    const IntNBitSplitEmbeddingWeightedBagsInst *I) {
  auto out = getTensor(I->getDest());
  auto devWeights = getTensor(I->getDevWeights());
  auto uvmWeights = getTensor(I->getUvmWeights());
  ...
  if (I->getIndiceWeight()->getElementType() == ElemKind::FloatTy) {
    fwdIntNBitSplitEmbeddingWeightedBagsImpl<IndexTy, OutputTy, WeightsOffsetTy,
                                             float>(
        out, devWeights, uvmWeights, weightsPlacements, weightsTys, dimOffsets,
        indices, offsets, weightsOffsets, poolingMode, indiceWeights, totalDims,
        outputDType);
  } else if (I->getIndiceWeight()->getElementType() == ElemKind::Float16Ty) {
    fwdIntNBitSplitEmbeddingWeightedBagsImpl<IndexTy, OutputTy, WeightsOffsetTy,
                                             float16_t>(
        out, devWeights, uvmWeights, weightsPlacements, weightsTys, dimOffsets,
        indices, offsets, weightsOffsets, poolingMode, indiceWeights, totalDims,
    ...
    assert(false && "IndiceWeight can be Float32 or Float16 only");
  }
}

template <typename IndexTy, typename OutputTy, typename WeightsOffsetTy,
          typename IndiceWeightTy>
void BoundVTAInterpreterFunction::fwdIntNBitSplitEmbeddingWeightedBagsImpl(
  ...
  auto outH = out->getHandle<OutputTy>();
  ...
  for (int32_t t = 0; t < numTables; t++) {
    ...
    auto numDimBytes =
        ::VTAInterpreter::IntNBitSplitEmbeddingBagsHelper::paddedRowSizeInBytes(
            numDims, weightTy);
    ...
    for (int32_t b = 0; b < numBatches; b++) {
      ...
      for (int32_t d = 0; d < numDims; d++) {
        ...
        for (IndexTy i = indicesStart; i < indicesEnd; i++) {
          ...
          int64_t idxData = idxRow +
                            ::VTAInterpreter::IntNBitSplitEmbeddingBagsHelper::
                                unpaddedRowSizeInBytes(d, weightTy);
          assert(idxData < maxWeightsBytes &&
                 "Index shall be within weights boundary.");
          sum = ::VTAInterpreter::IntNBitSplitEmbeddingBagsHelper::add(
              sum, &weightsH.raw(idxRow), &weightsH.raw(idxData), weightTy,
              d % 2,
              indiceWeights ? static_cast<float>(indiceWeightsH->raw(i)) : 1.0);
        }
        int64_t idxOut = b * totalDims + dimStart + d;
        if (poolingMode == SplitEmbeddingPoolingMode::EP_MEAN &&
            indicesEnd > indicesStart) {
          OutputTy scale = (indicesEnd - indicesStart);
          outH.raw(idxOut) = sum / scale;
        } else {
          outH.raw(idxOut) = sum;
        }
      }
...
void BoundVTAInterpreterFunction::fwdPermutePooledEmbeddingsInst(
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdPermutedPooledEmbeddingsInstImpl(
    const PermutePooledEmbeddingsInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAInterpreterFunction::fwdArgMaxInst(const ArgMaxInst *I) {
...
void BoundVTAInterpreterFunction::fwdArgMinInst(const ArgMinInst *I) {
  auto inpT = getTensor(I->getSrc());
  auto outT = getTensor(I->getDest());
  size_t axis = I->getAxis();
  auto inpElemType = inpT->getElementType();
...
void BoundVTAInterpreterFunction::fwdAllocActivationInst(
    const AllocActivationInst *I) {
  getOrCreateTensor(I);
}

void BoundVTAInterpreterFunction::fwdDeallocActivationInst(
...
/// tensor.
void BoundVTAInterpreterFunction::fwdDebugPrintInst(const DebugPrintInst *I) {
  auto *V = I->getSrc();
  auto *T = getTensor(V);
...
void BoundVTAInterpreterFunction::fwdTraceEventInst(const TraceEventInst *I) {
...
void BoundVTAInterpreterFunction::fwdInstrumentInst(const InstrumentInst *I) {
...
//===----------------------------------------------------------------------===//
void BoundVTAInterpreterFunction::fwdQuantizationProfileInst(
...
void BoundVTAInterpreterFunction::fwdQuantizeInst(const glow::QuantizeInst *I) {
...
void BoundVTAInterpreterFunction::fwdDequantizeInst(
...
template <class eTy>
void BoundVTAInterpreterFunction::fwdRescaleQuantizedInstImpl(
...
void BoundVTAInterpreterFunction::fwdRescaleQuantizedInst(
...
void BoundVTAInterpreterFunction::fwdIntLookupTableInst(
...
void BoundVTAInterpreterFunction::fwdLookupTableInst(const LookupTableInst *I) {
  llvm_unreachable("LookupTable instruction is not supported yet");
}

void BoundVTAInterpreterFunction::fwdConvertToInst(
    const glow::ConvertToInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdBatchedPairwiseDotProductInstImpl(
...
void BoundVTAInterpreterFunction::fwdBatchedPairwiseDotProductInst(
    const BatchedPairwiseDotProductInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdBatchedPairwiseDotProductGradInstImpl(
...
void BoundVTAInterpreterFunction::fwdBatchedPairwiseDotProductGradInst(
    const BatchedPairwiseDotProductGradInst *I) {
...
template <typename ElemTy>
void BoundVTAInterpreterFunction::fwdFlipInstImpl(const FlipInst *I) {
...
void BoundVTAInterpreterFunction::fwdFlipInst(const FlipInst *I) {
...
template <typename T>
void BoundVTAInterpreterFunction::fwdNonMaxSuppressionInstImpl(
...
void BoundVTAInterpreterFunction::fwdNonMaxSuppressionInst(
    glow::NonMaxSuppressionInst const *I) {
...
void BoundVTAInterpreterFunction::fwdTFLiteCustomOperatorInst(
    glow::TFLiteCustomOperatorInst const *I) {
  DCHECK(!"TFLiteCustomOperator is not implemented for Interpreter!");
}

void BoundVTAInterpreterFunction::fwdTFLiteDetectionPostProcessInst(
...
void BoundVTAInterpreterFunction::fwdAudioSpectrogramInstFloatImpl(
...
void BoundVTAInterpreterFunction::fwdAudioSpectrogramInst(
    glow::AudioSpectrogramInst const *I) {
...
void BoundVTAInterpreterFunction::fwdMFCCInstFloatImpl(
    glow::MFCCInst const *I) {
...
void BoundVTAInterpreterFunction::fwdMFCCInst(glow::MFCCInst const *I) {
  auto spectrogramTy = I->getSpectrogram()->getElementType();
...
void BoundVTAInterpreterFunction::fwdROIAlignInst(glow::ROIAlignInst const *I) {
...
void BoundVTAInterpreterFunction::fwdExternalFunctionCallInst(
