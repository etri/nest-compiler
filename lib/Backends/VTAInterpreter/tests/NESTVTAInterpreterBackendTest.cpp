...
TEST(inferVTAConvReluNet, conv_relu_fusion_test) {
  ...
  // EXPECT_TRUE(out1.getHandle().raw(0) ==
  // (double)out2.getHandle<int8_t>().raw(0));
...
TEST(inferVTALayoutConvReluNet, VTAResnetTest1) {
  ...
  // inferVTAConvNettemp2(&inputs, &kernel, &bias, &out2, "VTAInterpreter"); //
  // original code out2.toBin("golden_vtainterpreter");
...
// transformPostLoweirng.optimizeVTAConv: 6dim VTAConv graph optimization
// testing
TEST(inferVTALayoutConvReluNet, VTAConvGraphOptzTest) {
  ...
  inferVTAConvReluNet2(&inputs, &kernel, &bias, &out2, 3, 1, 1,
                       "VTAInterpreter");
