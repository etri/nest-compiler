...
void VTAInterpreterFunction::collectConstants(const Module *module) {
...
void VTAInterpreterFunction::addConstant(std::string name, Tensor *T) {
  Tensor *newTensor = new Tensor;
  newTensor->assign(T);
  constants_[name] = newTensor;
}

Error VTAInterpreterFunction::execute(ExecutionContext *context) {
  BoundVTAInterpreterFunction boundFunc(constants_);
...
void VTAInterpreterFunction::translateTraceEvents(
...
BoundVTAInterpreterFunction::~BoundVTAInterpreterFunction() {
...
Tensor *BoundVTAInterpreterFunction::getTensor(const Value *v) const {
...
Tensor *BoundVTAInterpreterFunction::getOrCreateTensor(const Value *v) {
  auto ie = externalTensors_.find(v);
...
Tensor *BoundVTAInterpreterFunction::getOrCreateUnownedTensor(
...
void BoundVTAInterpreterFunction::deleteTensor(const Value *v) {
  auto it = tensors_.find(v);
  if (it == tensors_.end()) {
    return;
  }
...
Error BoundVTAInterpreterFunction::execute(IRFunction *F,
  ...
  // Dispatch the VTAInterpreter on each instruction in the program.
