 ...
 */
#ifndef GLOW_BACKENDS_VTAInterpreter_VTAInterpreterDEVICEMANAGER_H
#define GLOW_BACKENDS_VTAInterpreter_VTAInterpreterDEVICEMANAGER_H
...
class VTAInterpreterDeviceManager : public QueueBackedDeviceManager {
  ...
  static constexpr const char *kDevicesUsedVTAInterpreter =
      "glow.devices_used.VTAInterpreter";

public:
  explicit VTAInterpreterDeviceManager(const DeviceConfig &config)
      : QueueBackedDeviceManager(config) {
    statsExporterRegistry_->incrementCounter(kDevicesUsedVTAInterpreter);
    exportMemoryCounters();
  }

  ~VTAInterpreterDeviceManager() override {
    statsExporterRegistry_->incrementCounter(kDevicesUsedVTAInterpreter, -1);
    zeroMemoryCounters();
  }
...
#endif // GLOW_BACKENBDS_VTAInterpreter_VTAInterpreterDEVICEMANAGER_H
