...
void BoundVTAFunction::fwdConvolutionInstFloatImpl(
...
template <typename ElemTy>
void BoundVTAFunction::fwdConvTransposeInstFloatImpl(
    Value *inV, Value *outV, Value *filterV, Value *biasV,
    llvm::ArrayRef<unsigned_t> kernelSizes, llvm::ArrayRef<unsigned_t> strides,
    llvm::ArrayRef<unsigned_t> pads, size_t group,
    llvm::ArrayRef<unsigned_t> dilation) {
...
void BoundVTAFunction::fwdConvTransposeInst(const ConvTransposeInst *I) {
...
void BoundVTAFunction::fwdFusionGroupInst(const glow::FusionGroupInst *p) {
  llvm_unreachable(
      "Fusion Group instruction not supported on Inrerpretter backend\n");
}

void BoundVTAFunction::fwdConcatInst(const ConcatInst *I) {
  (void)I;
  // TODO
  llvm_unreachable("not yet implemented");
}

void BoundVTAFunction::fwdConvolutionGradInst(const ConvolutionGradInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdConvolution3DInstFloatImpl(
    Value *inV, Value *outV, Value *filterV, Value *biasV,
    llvm::ArrayRef<unsigned_t> kernelSizes, llvm::ArrayRef<unsigned_t> strides,
    llvm::ArrayRef<unsigned_t> pads, size_t group) {
  staticAssertFloatingPointType(ElemTy);
...
template <typename ElemTy, typename AccumulatorTy, typename BiasElemTy>
void BoundVTAFunction::fwdConvolution3DInstQuantizedImpl(
    Value *inV, Value *outV, Value *filterV, Value *biasV,
    llvm::ArrayRef<unsigned_t> kernelSizes, llvm::ArrayRef<unsigned_t> strides,
    llvm::ArrayRef<unsigned_t> pads, size_t group) {
  auto inW = getWeightHandle<ElemTy>(inV);
        ...
        for (dim_t at = 0; at < odim.t; t += sdim.temporal_frames, at++) {
          ...
          for (dim_t ax = 0; ax < odim.h; x += sdim.height, ax++) {
            ...
            for (dim_t ay = 0; ay < odim.w; y += sdim.width, ay++) {
              ...
              // Scale the bias to match the scale of the matrix
              // multiplication.
...
void BoundVTAFunction::fwdConvolution3DInst(const Convolution3DInst *I) {
  auto kernelSizes = I->getKernels();
  auto pads = I->getPads();
  auto strides = I->getStrides();
  size_t group = I->getGroup();
...
void BoundVTAFunction::fwdConvolution3DGradInst(
...
//===----------------------------------------------------------------------===//
template <typename ElemTy, typename AccumulatorTy, typename BiasElemTy>
void BoundVTAFunction::fwdChannelwiseQuantizedConv2DInstImpl(
...
template <typename ElemTy, typename AccumulatorTy, typename BiasElemTy>
void BoundVTAFunction::fwdChannelwiseQuantizedConv3DInstImpl(
    const ChannelwiseQuantizedConvolutionInst *I) {
  auto inW = getWeightHandle<ElemTy>(I->getSrc());
  auto outW = getWeightHandle<ElemTy>(I->getDest());
  auto filterW = getWeightHandle<ElemTy>(I->getFilter());
...
void BoundVTAFunction::fwdChannelwiseQuantizedConvolutionInst(
    const ChannelwiseQuantizedConvolutionInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdBatchNormalizationFloatImpl(
...
void BoundVTAFunction::fwdBatchNormalizationI8Impl(
    const BatchNormalizationInst *I, int numDims) {
  ...
  // See QuantizedOpKernels.cpp/q_batch_norm_kernel() for FBGEMM
  // implementation
...
void BoundVTAFunction::fwdBatchNormalizationInst(
...
template <typename ElemTy>
void BoundVTAFunction::fwdLayerNormalizationInstFloatImpl(
...
void BoundVTAFunction::fwdLayerNormalizationInst(
...
void BoundVTAFunction::fwdMaxPoolWithArgmaxInst(
...
template <typename ElemTy>
void BoundVTAFunction::fwdAvgPoolInstFloatImpl(const AvgPoolInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
template <typename ElemTy>
void BoundVTAFunction::fwdAvgPool3DInstFloatImpl(const AvgPoolInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdAvgPool3DInstI8Impl(const AvgPoolInst *I) {
  ShapeNTHWC odim(I->getDest()->dims());
  ShapeNTHWC idim(I->getSrc()->dims());
...
template <typename ElemTy>
void BoundVTAFunction::fwdAdaptiveAvgPoolInstFloatImpl(
...
void BoundVTAFunction::fwdAdaptiveAvgPoolInstI8Impl(
    const AdaptiveAvgPoolInst *I) {
  ShapeNHWC odim(I->getDest()->dims());
  ShapeNHWC idim(I->getSrc()->dims());
...
void BoundVTAFunction::fwdAdaptiveAvgPoolInst(const AdaptiveAvgPoolInst *I) {
  if (I->getSrc()->getType()->isQuantizedType()) {
...
void BoundVTAFunction::fwdAdaptiveAvgPoolGradInst(
...
void BoundVTAFunction::fwdMaxPoolWithArgmaxGradInst(
...
void BoundVTAFunction::fwdAvgPool2DGradInst(const AvgPoolGradInst *I) {
  auto inG = getWeightHandle(I->getSrcGrad());
  auto outW = getWeightHandle(I->getDest());
  auto outG = getWeightHandle(I->getDestGrad());
...
void BoundVTAFunction::fwdAvgPool3DGradInst(const AvgPoolGradInst *I) {
  auto inG = getWeightHandle(I->getSrcGrad());
  auto outW = getWeightHandle(I->getDest());
  auto outG = getWeightHandle(I->getDestGrad());
...
void BoundVTAFunction::fwdAvgPoolGradInst(const AvgPoolGradInst *I) {
  bool isConv3D = is3DData(ConvolutionLayout(I->getLayout()));
...
void BoundVTAFunction::fwdClipInst(const ClipInst *) {
  DCHECK(!"Found ClipInst but Clip is lowered on Interpreter");
}

void BoundVTAFunction::fwdLeakyReluInst(const LeakyReluInst *) {
  DCHECK(!"Found LeakyReluInst but LeakyRelu is lowered on Interpreter");
}

void BoundVTAFunction::fwdHardSwishInst(const HardSwishInst *) {
  DCHECK(!"Found HardSwishInst but HardSwish is lowered on Interpreter");
}

template <typename ElemTy>
void BoundVTAFunction::fwdSigmoidInstFloatImpl(const SigmoidInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdSigmoidInst(const SigmoidInst *I) {
  dispatchFloatingPointImpl(fwdSigmoidInstFloatImpl,
                            I->getSrc()->getElementType(), I);
}

template <typename ElemTy>
void BoundVTAFunction::fwdTanhInstFloatImpl(const TanhInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdTanhInst(const TanhInst *I) {
  dispatchFloatingPointImpl(fwdTanhInstFloatImpl, I->getSrc()->getElementType(),
                            I);
}

template <typename ElemTy>
void BoundVTAFunction::fwdSoftPlusInstFloatImpl(const SoftPlusInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdSoftPlusInst(const SoftPlusInst *I) {
...
void BoundVTAFunction::fwdSoftMaxGradInst(const SoftMaxGradInst *I) {
  auto inG = getWeightHandle(I->getSrcGrad());
  auto idim = inG.dims();
...
template <typename ElemTy>
void BoundVTAFunction::fwdLogSoftMaxInstImpl(const LogSoftMaxInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdLogSoftMaxInst(const LogSoftMaxInst *I) {
  dispatchFloatingPointImpl(fwdLogSoftMaxInstImpl,
                            I->getSrc()->getElementType(), I);
}

template <typename ElemTy>
void BoundVTAFunction::fwdCrossEntropyLossInstFloatImpl(
...
void BoundVTAFunction::fwdCrossEntropyLossInst(const CrossEntropyLossInst *I) {
  dispatchFloatingPointImpl(fwdCrossEntropyLossInstFloatImpl,
                            I->getP()->getElementType(), I);
}

void BoundVTAFunction::fwdCrossEntropyLossGradInst(
...
void BoundVTAFunction::fwdCopyInst(const CopyInst *I) {
...
void BoundVTAFunction::fwdTouchInst(const glow::TouchInst *) {
...
void BoundVTAFunction::fwdExtractTensorInst(const glow::ExtractTensorInst *I) {
  Tensor *outT = getTensor(I->getDest());
  Tensor *inT = getTensor(I->getSrc());
  ElemKind k = outT->getElementType();
#define TYPED_INSERT(TY, TYPEKIND)                                             \
...
template <typename ElemTy>
void BoundVTAFunction::fwdGatherInstImpl(const glow::GatherInst *I) {
...
void BoundVTAFunction::fwdGatherInst(const glow::GatherInst *I) {
...
void BoundVTAFunction::fwdGatherElementsInstImpl(
...
void BoundVTAFunction::fwdGatherElementsInst(
    const glow::GatherElementsInst *I) {
  switch (I->getIndices()->getElementType()) {
  case ElemKind::Int64ITy:
...
template <typename ElemTy>
void BoundVTAFunction::fwdGatherNDInstImpl(const glow::GatherNDInst *I) {
...
void BoundVTAFunction::fwdGatherNDInst(const glow::GatherNDInst *I) {
  switch (I->getIndices()->getElementType()) {
  case ElemKind::Int64ITy:
...
template <typename ElemTy>
void BoundVTAFunction::fwdGatherRangesInstImpl(
...
void BoundVTAFunction::fwdGatherRangesInst(const glow::GatherRangesInst *I) {
...
void BoundVTAFunction::fwdScatterDataInstCopyImpl(
...
template <typename ElemTy, typename IndicesElemTy>
void BoundVTAFunction::fwdScatterDataInstAddFloatImpl(
    const glow::ScatterDataInst *I) {
  Tensor *dataT = getTensor(I->getData());
  Tensor *indicesT = getTensor(I->getIndices());
  Tensor *slicesT = getTensor(I->getSlices());
...
template <typename ElemTy, typename IndicesElemTy>
void BoundVTAFunction::fwdScatterDataInstAddQuantizedImpl(
    const glow::ScatterDataInst *I) {
  Tensor *dataT = getTensor(I->getData());
  Tensor *indicesT = getTensor(I->getIndices());
  Tensor *slicesT = getTensor(I->getSlices());
...
void BoundVTAFunction::fwdScatterDataInst(const glow::ScatterDataInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdBatchOneHotImpl(const glow::BatchOneHotInst *I) {
...
void BoundVTAFunction::fwdBatchOneHotInst(const glow::BatchOneHotInst *I) {
  switch (I->getData()->getElementType()) {
  case ElemKind::Int64ITy:
...
template <typename ElemTy>
void BoundVTAFunction::fwdSpaceToDepthInstImpl(
...
void BoundVTAFunction::fwdSpaceToDepthInst(const glow::SpaceToDepthInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdResizeNearestInstImpl(const ResizeNearestInst *I) {
  auto inW = getWeightHandle<ElemTy>(I->getSrc());
...
void BoundVTAFunction::fwdResizeNearestInst(const ResizeNearestInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdResizeBilinearInstImpl(const ResizeBilinearInst *I) {
  auto inW = getWeightHandle<ElemTy>(I->getSrc());
  auto scale = I->getScale();
  auto outW = getWeightHandle<ElemTy>(I->getDest());
...
void BoundVTAFunction::fwdResizeBilinearInst(const ResizeBilinearInst *I) {
  if (getTensor(I->getSrc())->getType().isQuantizedType()) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdLocalResponseNormalizationInstFloatImpl(
...
void BoundVTAFunction::fwdLocalResponseNormalizationInst(
    const LocalResponseNormalizationInst *I) {
  dispatchFloatingPointImpl(fwdLocalResponseNormalizationInstFloatImpl,
                            I->getSrc()->getElementType(), I);
}

void BoundVTAFunction::fwdLocalResponseNormalizationGradInst(
...
void BoundVTAFunction::fwdBucketizeInst(const BucketizeInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdElementAddInstArithmeticImpl(
...
template <typename ElemTy>
void BoundVTAFunction::fwdElementMulInstArithmeticImpl(
...
void BoundVTAFunction::fwdElementMulInst(const ElementMulInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
    auto lhsTy = I->getLHS()->getType();
    auto rhsTy = I->getRHS()->getType();
    auto destTy = I->getDest()->getType();
...
void BoundVTAFunction::fwdElementFmodInst(const ElementFmodInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
    auto destTy = I->getDest()->getType();
    auto lhsTy = I->getLHS()->getType();
    auto rhsTy = I->getRHS()->getType();
...
template <typename ElemTy>
void BoundVTAFunction::fwdElementMaxInstArithmeticImpl(
...
template <typename ElemTy>
void BoundVTAFunction::fwdElementMinInstArithmeticImpl(
...
void BoundVTAFunction::fwdElementMinInst(const ElementMinInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
    auto lhsTy = I->getLHS()->getType();
    auto rhsTy = I->getRHS()->getType();
    auto destTy = I->getDest()->getType();
...
template <typename ElemTy>
void BoundVTAFunction::fwdElementBitwiseOrInstImpl(
...
void BoundVTAFunction::fwdElementBitwiseOrInst(const ElementBitwiseOrInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdElementBitwiseAndInstImpl(
...
void BoundVTAFunction::fwdElementBitwiseAndInst(
    const ElementBitwiseAndInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdElementBitwiseXorInstImpl(
...
void BoundVTAFunction::fwdElementBitwiseXorInst(
    const ElementBitwiseXorInst *I) {
...
//                              Logical operations
//===----------------------------------------------------------------------===//
void BoundVTAFunction::fwdElementNotInst(const ElementNotInst *I) {
...
void BoundVTAFunction::fwdElementAndInst(const ElementAndInst *I) {
...
void BoundVTAFunction::fwdElementOrInst(const ElementOrInst *I) {
  auto lhsW = getWeightHandle<bool>(I->getLHS());
  auto rhsW = getWeightHandle<bool>(I->getRHS());
  auto outW = getWeightHandle<bool>(I->getDest());
  for (size_t i = 0, e = outW.size(); i < e; ++i) {
...
void BoundVTAFunction::fwdElementXorInst(const ElementXorInst *I) {
  auto lhsW = getWeightHandle<bool>(I->getLHS());
  auto rhsW = getWeightHandle<bool>(I->getRHS());
  auto outW = getWeightHandle<bool>(I->getDest());
  for (size_t i = 0, e = outW.size(); i < e; ++i) {
...
void BoundVTAFunction::fwdUnaryArithmeticImpl(
...
void BoundVTAFunction::fwdElementBitwiseNotInst(
...
void BoundVTAFunction::fwdElementAbsInst(const ElementAbsInst *I) {
  auto func = [](float x) -> float { return std::abs(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAFunction::fwdElementNegInst(const ElementNegInst *I) {
  auto func = [](float x) -> float { return -x; };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAFunction::fwdElementFloorInst(const ElementFloorInst *I) {
...
void BoundVTAFunction::fwdElementCeilInst(const ElementCeilInst *I) {
  auto func = [](float x) -> float { return std::ceil(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAFunction::fwdElementTruncateInst(const ElementTruncateInst *I) {
  auto func = [](float x) -> float { return std::trunc(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAFunction::fwdElementRoundInst(const ElementRoundInst *I) {
...
void BoundVTAFunction::fwdElementSqrtInst(const ElementSqrtInst *I) {
  auto func = [](float x) -> float { return std::sqrt(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAFunction::fwdElementRsqrtInst(const ElementRsqrtInst *I) {
  auto func = [](float x) -> float { return 1 / std::sqrt(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAFunction::fwdElementReciprocalInst(
    const ElementReciprocalInst *I) {
  auto func = [](float x) -> float { return 1 / x; };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAFunction::fwdElementSinInst(const ElementSinInst *I) {
  auto func = [](float x) -> float { return std::sin(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAFunction::fwdElementCosInst(const ElementCosInst *I) {
  auto func = [](float x) -> float { return std::cos(x); };
  dispatchImpl(fwdUnaryArithmeticImpl, I->getSrc()->getElementType(), I, func);
}

void BoundVTAFunction::fwdElementErfInst(const ElementErfInst *I) {
...
void BoundVTAFunction::fwdElementCmpHelperImpl(
...
template <typename ElemTy, typename ElemOffsetTy, typename ElemScaleTy,
          typename CmpTy>
void BoundVTAFunction::fwdElementCmpLTEInstImpl(const ElementCmpLTEInst *I) {
...
void BoundVTAFunction::fwdElementCmpLTEInst(const ElementCmpLTEInst *I) {
...
template <typename ElemTy, typename ElemOffsetTy, typename ElemScaleTy,
          typename CmpTy>
void BoundVTAFunction::fwdElementCmpEQInstImpl(const ElementCmpEQInst *I) {
  auto cmpHelper = [](CmpTy LHS, CmpTy RHS) -> bool { return LHS == RHS; };
  fwdElementCmpHelperImpl<ElemTy, ElemOffsetTy, ElemScaleTy, CmpTy,
                          ElementCmpEQInst>(I, cmpHelper);
}

void BoundVTAFunction::fwdElementCmpEQInst(const ElementCmpEQInst *I) {
  auto *T = getTensor(I->getLHS());
...
template <typename ElemTy, typename ElemOffsetTy, typename ElemScaleTy,
          typename CmpTy>
void BoundVTAFunction::fwdElementCmpNEQInstImpl(const ElementCmpNEQInst *I) {
  auto cmpHelper = [](CmpTy LHS, CmpTy RHS) -> bool { return !(LHS == RHS); };
  fwdElementCmpHelperImpl<ElemTy, ElemOffsetTy, ElemScaleTy, CmpTy,
                          ElementCmpNEQInst>(I, cmpHelper);
}

void BoundVTAFunction::fwdElementCmpNEQInst(const ElementCmpNEQInst *I) {
  auto *T = getTensor(I->getLHS());
...
template <typename ElemTy, typename ElemOffsetTy, typename ElemScaleTy,
          typename CmpTy>
void BoundVTAFunction::fwdElementCmpLTInstImpl(const ElementCmpLTInst *I) {
  auto cmpHelper = [](CmpTy LHS, CmpTy RHS) -> bool { return LHS < RHS; };
  fwdElementCmpHelperImpl<ElemTy, ElemOffsetTy, ElemScaleTy, CmpTy,
                          ElementCmpLTInst>(I, cmpHelper);
}

void BoundVTAFunction::fwdElementCmpLTInst(ElementCmpLTInst const *I) {
  auto *T = getTensor(I->getLHS());
  if (T->getType().isQuantizedType()) {
    switch (T->getElementType()) {
    case ElemKind::Int8QTy:
...
template <typename ElemTy>
void BoundVTAFunction::fwdElementPowInstFloatImpl(const ElementPowInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdElementPowInstI8Impl(const ElementPowInst *I) {
  assert(getTensor(I->getLHS())->getType().isQuantizedType() &&
...
void BoundVTAFunction::fwdElementPowInst(const glow::ElementPowInst *I) {
  auto *T = getTensor(I->getLHS());
  if (T->getType().isQuantizedType()) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdElementIsNaNInstFloatImpl(const ElementIsNaNInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdElementIsNaNInst(const glow::ElementIsNaNInst *I) {
  dispatchFloatingPointImpl(fwdElementIsNaNInstFloatImpl,
                            I->getSrc()->getElementType(), I);
}

template <typename ElemTy>
void BoundVTAFunction::fwdElementLogInstFloatImpl(const ElementLogInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdElementLogInst(const ElementLogInst *I) {
  dispatchFloatingPointImpl(fwdElementLogInstFloatImpl,
                            I->getSrc()->getElementType(), I);
}

template <typename ElemTy>
void BoundVTAFunction::fwdElementExpInstFloatImpl(const ElementExpInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdElementExpInst(const ElementExpInst *I) {
  dispatchFloatingPointImpl(fwdElementExpInstFloatImpl,
                            I->getSrc()->getElementType(), I);
}

void BoundVTAFunction::fwdNonZeroInst(const NonZeroInst *I) {
  auto *T = getTensor(I->getDest());
  T->zero();
...
template <typename ElemTy>
void BoundVTAFunction::fwdElementSelectInstFloatImpl(
...
void BoundVTAFunction::fwdElementSelectInst(const glow::ElementSelectInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
    auto destTy = I->getDest()->getType();
    auto lhsTy = I->getLHS()->getType();
    auto rhsTy = I->getRHS()->getType();
...
template <typename ElemTy>
void BoundVTAFunction::fwdModuloInstImpl(glow::ModuloInst const *I) {
...
void BoundVTAFunction::fwdModuloInst(glow::ModuloInst const *I) {
  dispatchIndexTypeImpl(fwdModuloInstImpl, I->getSrc()->getElementType(), I);
}

///=============== Trigonometric Operators===============
template <typename ElemTy, typename InstKind>
void BoundVTAFunction::fwdUnaryTrigonometricImpl(
    const InstKind *I, std::function<float(float)> func) {
  Value *inpV = I->getSrc();
  Value *outV = I->getDest();
  auto inpTy = inpV->getType();
...
void BoundVTAFunction::fwdElementAcosInst(const ElementAcosInst *I) {
  auto func = [](float x) -> float { return std::acos(x); };
  dispatchImpl(fwdUnaryTrigonometricImpl, I->getSrc()->getElementType(), I,
               func);
}

void BoundVTAFunction::fwdElementAsinInst(const ElementAsinInst *I) {
  auto func = [](float x) -> float { return std::asin(x); };
  dispatchImpl(fwdUnaryTrigonometricImpl, I->getSrc()->getElementType(), I,
               func);
}

void BoundVTAFunction::fwdElementAtanInst(const ElementAtanInst *I) {
...
void BoundVTAFunction::fwdMatMulInstQuantizedImpl(const glow::MatMulInst *I) {
  assert(getTensor(I->getLHS())->getType().isQuantizedType());
...
template <typename ElemTy>
void BoundVTAFunction::fwdMatMulInstFloatImpl(const MatMulInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
template <typename ElemTy>
void BoundVTAFunction::fwdBatchMatMulInstFloatImpl(const BatchMatMulInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdMatMulInst(const glow::MatMulInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
...
void BoundVTAFunction::fwdBatchMatMulInst(const glow::BatchMatMulInst *I) {
  if (getTensor(I->getLHS())->getType().isQuantizedType()) {
...
void BoundVTAFunction::fwdReluGradInst(const glow::ReluGradInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdFullyConnectedInstFloatImpl(
...
void BoundVTAFunction::fwdDynRowwiseQuantizedFullyConnectedInstImpl(
...
void BoundVTAFunction::fwdDynRowwiseQuantizedFullyConnectedInstPreimpl(
...
void BoundVTAFunction::fwdDynamicRowwiseQuantizedFullyConnectedInst(
...
void BoundVTAFunction::fwdDynamicQuantizedFullyConnectedInst(
...
//===----------------------------------------------------------------------===//
template <typename ElemTy, typename AccumulatorTy, typename BiasElemTy>
void BoundVTAFunction::fwdRowwiseQuantizedFullyConnectedInstImpl(
...
void BoundVTAFunction::fwdRowwiseQuantizedFullyConnectedInst(
...
template <typename ElemTy>
void BoundVTAFunction::fwdBatchedAddInstFloatImpl(
...
void BoundVTAFunction::fwdBatchedAddInst(const glow::BatchedAddInst *I) {
...
#define DEFINE_REDUCEADDPROD_INST_IMPL(func, init, op, inst)                   \
  template <typename ElemTy>                                                   \
  void BoundVTAFunction::fwdBatched##func##inst(                               \
...
void BoundVTAFunction::fwdBatchedReduceAddInst(
...
void BoundVTAFunction::fwdBatchedReduceProdInst(
...
#define DEFINE_REDUCEMINMAX_INST_IMPL(func, compare)                           \
  template <typename ElemTy>                                                   \
  void BoundVTAFunction::fwdBatched##func##InstImpl(                           \
...
template <typename ElemTy>
void BoundVTAFunction::fwdCumSumInstImpl(Value *input, Value *dest, int64_t dim,
...
void BoundVTAFunction::fwdCumSumInst(glow::CumSumInst const *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdLengthsSumInstFloatImpl(const LengthsSumInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdLengthsSumInst(const LengthsSumInst *I) {
  dispatchFloatingPointImpl(fwdLengthsSumInstFloatImpl,
                            I->getData()->getElementType(), I)
}

template <typename TI>
void BoundVTAFunction::fwdSparseLengthsSumInstI8Impl(
...
void BoundVTAFunction::fwdSparseLengthsSumInstFloatImpl(
    const SparseLengthsSumInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdSparseLengthsSumInst(const SparseLengthsSumInst *I) {
...
template <typename ElemTy, typename TI>
void BoundVTAFunction::fwdSparseLengthsWeightedSumInstFloatImpl(
...
template <typename TI>
void BoundVTAFunction::fwdSparseLengthsWeightedSumInstI8Impl(
    const SparseLengthsWeightedSumInst *I) {
...
void BoundVTAFunction::fwdSparseLengthsWeightedSumGradInst(
...
void BoundVTAFunction::fwdEmbeddingBagInstFloatImpl(const EmbeddingBagInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdEmbeddingBagInst(const EmbeddingBagInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdEmbeddingInstImpl(Tensor *wtT, Tensor *indT,
                                            Tensor *outT, int64_t padIdx,
                                            bool sparse, bool scale,
                                            dim_t embedding_dim) {
...
void BoundVTAFunction::fwdEmbeddingInst(const EmbeddingInst *I) {
...
void BoundVTAFunction::fwdRowwiseQuantizedSparseLengthsWeightedSumImpl(
...
void BoundVTAFunction::fwdRowwiseQuantizedSparseLengthsWeightedSumInst(
    const RowwiseQuantizedSparseLengthsWeightedSumInst *I) {
...
template <typename T, typename AccumT, typename TI>
void BoundVTAFunction::fwdFusedRowwiseQuantizedSparseLengthsWeightedSumImpl(
...
void BoundVTAFunction::fwdFusedRowwiseQuantizedSparseLengthsWeightedSumInst(
    const FusedRowwiseQuantizedSparseLengthsWeightedSumInst *I) {
  const auto ity = I->getIndices()->getElementType();
...
void BoundVTAFunction::fwdFusedRowwiseQuantizedSparseLengthsSumInst(
    const FusedRowwiseQuantizedSparseLengthsSumInst *I) {
  llvm_unreachable("Not supported");
}

template <typename T, typename AccumT, typename IndexT>
void BoundVTAFunction::fwdEmbeddingBagByteRowwiseOffsetsImpl(
...
void BoundVTAFunction::fwdEmbeddingBagByteRowwiseOffsetsInst(
    const EmbeddingBagByteRowwiseOffsetsInst *I) {
  const auto ity = I->getIndices()->getElementType();
  const bool fp32FusedScaleOffset =
      (I->getData()->getElementType() == ElemKind::UInt4FusedQTy) ||
...
void BoundVTAFunction::fwdLengthsToRangesInst(const LengthsToRangesInst *I) {
...
void BoundVTAFunction::fwdLengthsRangeFillInst(const LengthsRangeFillInst *I) {
...
void BoundVTAFunction::fwdGaussianFillInst(const GaussianFillInst *I) {
...
void BoundVTAFunction::fwdBatchSparseToDenseInstImpl2(
...
void BoundVTAFunction::fwdBatchSparseToDenseInstImpl1(
    const BatchSparseToDenseInst *I) {
...
void BoundVTAFunction::fwdBatchSparseToDenseInst(
    const BatchSparseToDenseInst *I) {
...
void BoundVTAFunction::fwdFillExamplesWithIndicatorInstImpl2(
...
template <typename ElemTy>
void BoundVTAFunction::fwdFillExamplesWithIndicatorInstImpl1(
    const FillExamplesWithIndicatorInst *I) {
...
void BoundVTAFunction::fwdFillExamplesWithIndicatorInst(
    const FillExamplesWithIndicatorInst *I) {
  dispatchArithmeticImpl(fwdFillExamplesWithIndicatorInstImpl1,
                         I->getDest()->getElementType(), I);
}
void BoundVTAFunction::fwdSparseToDenseMaskInst(
...
void BoundVTAFunction::fwdSparseLabelSplitInst(const SparseLabelSplitInst *I) {
  auto lengthsH = getTensor(I->getLengths())->getHandle<int32_t>();
  auto indicesH = getTensor(I->getIndices())->getHandle<int64_t>();
...
template <typename ElemTy>
void BoundVTAFunction::fwdCollectRpnProposalsInstImpl(
...
void BoundVTAFunction::fwdCollectRpnProposalsInst(
    const glow::CollectRpnProposalsInst *I) {
  dispatchFloatingPointImpl(fwdCollectRpnProposalsInstImpl,
                            I->getOperand(1).first->getElementType(), I);
}

void BoundVTAFunction::fwdTopKInst(const TopKInst *I) {
...
void BoundVTAFunction::fwdBatchedUnaryEmbeddingsBagsInst(
...
template <typename ElemTy, typename IndexType>
void BoundVTAFunction::fwdBatchedUnaryEmbeddingsBagsInstImpl(
    const BatchedUnaryEmbeddingsBagsInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdIntNBitSplitEmbeddingBagsInst(
...
void BoundVTAFunction::fwdIntNBitSplitEmbeddingWeightedBagsInst(
...
void BoundVTAFunction::fwdIntNBitSplitEmbeddingBagsInstImpl(
    const IntNBitSplitEmbeddingBagsInst *I) {
  auto out = getTensor(I->getDest());
...
template <typename IndexTy, typename OutputTy, typename WeightsOffsetTy>
void BoundVTAFunction::fwdIntNBitSplitEmbeddingWeightedBagsInstImpl(
    const IntNBitSplitEmbeddingWeightedBagsInst *I) {
  auto out = getTensor(I->getDest());
  auto devWeights = getTensor(I->getDevWeights());
  auto uvmWeights = getTensor(I->getUvmWeights());
...
void BoundVTAFunction::fwdIntNBitSplitEmbeddingWeightedBagsImpl(
  ...
  for (int32_t t = 0; t < numTables; t++) {
    ...
    auto numDimBytes =
        ::VTA::IntNBitSplitEmbeddingBagsHelper::paddedRowSizeInBytes(numDims,
                                                                     weightTy);
    ...
    for (int32_t b = 0; b < numBatches; b++) {
      ...
      for (int32_t d = 0; d < numDims; d++) {
        ...
        for (IndexTy i = indicesStart; i < indicesEnd; i++) {
          ...
          int64_t idxData =
              idxRow +
              ::VTA::IntNBitSplitEmbeddingBagsHelper::unpaddedRowSizeInBytes(
                  d, weightTy);
          assert(idxData < maxWeightsBytes &&
                 "Index shall be within weights boundary.");
          sum = ::VTA::IntNBitSplitEmbeddingBagsHelper::add(
...
void BoundVTAFunction::fwdPermutePooledEmbeddingsInst(
...
template <typename ElemTy>
void BoundVTAFunction::fwdPermutedPooledEmbeddingsInstImpl(
    const PermutePooledEmbeddingsInst *I) {
  staticAssertFloatingPointType(ElemTy);
...
void BoundVTAFunction::fwdArgMaxInst(const ArgMaxInst *I) {
...
void BoundVTAFunction::fwdArgMinInst(const ArgMinInst *I) {
  auto inpT = getTensor(I->getSrc());
  auto outT = getTensor(I->getDest());
  size_t axis = I->getAxis();
  auto inpElemType = inpT->getElementType();
...
void BoundVTAFunction::fwdTraceEventInst(const TraceEventInst *I) {
...
void BoundVTAFunction::fwdInstrumentInst(const InstrumentInst *I) {
...
//===----------------------------------------------------------------------===//
void BoundVTAFunction::fwdQuantizationProfileInst(
...
template <class eTy>
void BoundVTAFunction::fwdRescaleQuantizedInstImpl(
...
void BoundVTAFunction::fwdRescaleQuantizedInst(
...
void BoundVTAFunction::fwdIntLookupTableInst(const IntLookupTableInst *I) {
...
void BoundVTAFunction::fwdLookupTableInst(const LookupTableInst *I) {
  llvm_unreachable("LookupTable instruction is not supported yet");
}

void BoundVTAFunction::fwdConvertToInst(const glow::ConvertToInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdBatchedPairwiseDotProductInstImpl(
...
void BoundVTAFunction::fwdBatchedPairwiseDotProductInst(
    const BatchedPairwiseDotProductInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdBatchedPairwiseDotProductGradInstImpl(
...
void BoundVTAFunction::fwdBatchedPairwiseDotProductGradInst(
    const BatchedPairwiseDotProductGradInst *I) {
...
template <typename ElemTy>
void BoundVTAFunction::fwdFlipInstImpl(const FlipInst *I) {
...
void BoundVTAFunction::fwdFlipInst(const FlipInst *I) {
...
template <typename T>
void BoundVTAFunction::fwdNonMaxSuppressionInstImpl(
...
void BoundVTAFunction::fwdNonMaxSuppressionInst(
    glow::NonMaxSuppressionInst const *I) {
...
void BoundVTAFunction::fwdTFLiteCustomOperatorInst(
    glow::TFLiteCustomOperatorInst const *I) {
  DCHECK(!"TFLiteCustomOperator is not implemented for Interpreter!");
}

void BoundVTAFunction::fwdTFLiteDetectionPostProcessInst(
...
void BoundVTAFunction::fwdAudioSpectrogramInstFloatImpl(
...
void BoundVTAFunction::fwdAudioSpectrogramInst(
    glow::AudioSpectrogramInst const *I) {
...
void BoundVTAFunction::fwdMFCCInstFloatImpl(glow::MFCCInst const *I) {
...
void BoundVTAFunction::fwdMFCCInst(glow::MFCCInst const *I) {
  auto spectrogramTy = I->getSpectrogram()->getElementType();
...
template <typename T>
void BoundVTAFunction::fwdROIAlignInstFloatImpl(glow::ROIAlignInst const *I) {
...
void BoundVTAFunction::fwdROIAlignInst(glow::ROIAlignInst const *I) {
...
template <typename T>
void BoundVTAFunction::fwdBBoxTransformInstFloatImpl(
...
void BoundVTAFunction::fwdBBoxTransformInst(glow::BBoxTransformInst const *I) {
  dispatchFloatingPointImpl(fwdBBoxTransformInstFloatImpl,
                            I->getRois()->getElementType(), I);
}

void BoundVTAFunction::fwdExternalFunctionCallInst(
