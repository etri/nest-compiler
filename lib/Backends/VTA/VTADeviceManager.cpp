...
static llvm::cl::OptionCategory VTABackendCat("Glow VTA Backend Options");
static llvm::cl::opt<unsigned, /* ExternalStorage */ true> vtaMaxMemOpt(
    "vta-memory",
    llvm::cl::desc("VTA DeviceManager maximum memory in kilobytes"),
    llvm::cl::location(GlowVTAMemory), llvm::cl::cat(VTABackendCat));
...
uint64_t VTADeviceManager::getMaximumMemory() const { return maxMemoryBytes_; }
...
void VTADeviceManager::addNetworkImpl(const Module *module,
  ...
  for (const auto &func : functions) {
    ...
    VTAFunction *function = static_cast<VTAFunction *>(func.second);
...
void VTADeviceManager::transferStaticPlaceholderToDevice(
  ...
  for (auto functionName : it->second) {
    VTAFunction *func = static_cast<VTAFunction *>(functions_[functionName]);
