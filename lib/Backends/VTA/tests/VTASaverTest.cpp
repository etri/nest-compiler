...
static Constant *createQuantizedConstant(Module &mod,
  ...
  auto *P = mod.createConstant(tensor->getElementType(), tensor->dims(), scale,
                               offset, name);
  P->assign(tensor);
...
static Constant *createConstant(Module &mod, PlaceholderBindings &bindings,
                                Tensor *tensor, llvm::StringRef name) {
...
TEST(VTASaverTest, convTest) {
  ...
  inferVTAMultiConvNet(&inputs, &filter, &bias, &filter2, &bias2, &outVTA,
                       &outVTA2, "VTA");
...
TEST(VTASaverTest, resnetv10S1C0Test) {
  ...
  llvm::StringRef mainEntryName =
      "vtaResnetv10S1C0TestMainEntry"; // main function name
...
TEST(VTASaverTest, multievtaConvTest) {
  for (int i = 0; i < 4; i++) {
    ...
    std::string mainentryname =
        "multievtaConvTestMainEntry" + std::to_string(i);
    ...
    inputP = createQuantizedPlaceholder(M, bindings, &inputs, inType.getScale(),
                                        inType.getOffset(), inputPname.c_str());

    filterP =
        createQuantizedConstant(M, bindings, &filter, filterType.getScale(),
...
void conv_test_template(
    dim_t N, dim_t H, dim_t W, dim_t C, float input_scale, dim_t KN, dim_t KH,
    dim_t KW, dim_t KC, float filter_scale,
    // dim_t ON, dim_t OH, dim_t OW, dim_t OC, float output_scale,
    float bias_scale, int stride, int pad, int group, int input_low,
    int input_high, int filter_low, int filter_high, int bias_low,
    int bias_high, std::string test_name, Tensor &out1, Tensor &out2) {
  assert(KH == KW);
  PseudoRNG PRNG;
  ...
  OT1 = F->getParent()->uniqueType(out1.getElementType(), out1.dims(),
                                   outType1.getScale(), outType1.getOffset());
  auto *conv = F->createConv("conv", inputP, filterP, biasP, OT1, KH, stride,
                             pad, group);
  ...
  inferVTAConvNet(&inputs, &filter, &bias, &out2, KH, stride, pad,
                  "VTAInterpreter");
...
TEST(VTASaverTest, ResConv1Test) {
  ...
  conv_test_template(1, 56, 56, 64, 1, 64, 3, 3, 64, 1, 1, 1, 1, 1, 0, 10, -10,
                     10, 0, 32768, "vtaResConv1Test", out1, out2);
...
TEST(VTASaverTest, ResConv2Test) {
  ...
  conv_test_template(1, 56, 56, 64, 1, 128, 1, 1, 64, 1, 1, 2, 0, 1, 0, 10, -10,
                     10, 0, 32768, "vtaResConv2Test", out1, out2);
...
TEST(VTASaverTest, ResConv3Test) {
  ...
  conv_test_template(1, 56, 56, 64, 1, 128, 3, 3, 64, 1, 1, 2, 1, 1, 0, 10, -10,
                     10, 0, 32768, "vtaResConv3Test", out1, out2);
...
TEST(VTASaverTest, ResConv4Test) {
  ...
  conv_test_template(1, 28, 28, 128, 1, 128, 3, 3, 128, 1, 1, 1, 1, 1, 0, 10,
                     -10, 10, 0, 32768, "vtaResConv4Test", out1, out2);
...
TEST(VTASaverTest, ResConv5Test) {
  ...
  conv_test_template(1, 28, 28, 128, 1, 256, 1, 1, 128, 1, 1, 2, 0, 1, 0, 10,
                     -10, 10, 0, 32768, "vtaResConv5Test", out1, out2);
...
TEST(VTASaverTest, ResConv6Test) {
  ...
  conv_test_template(1, 28, 28, 128, 1, 256, 3, 3, 128, 1, 1, 2, 1, 1, 0, 10,
                     -10, 10, 0, 32768, "vtaResConv6Test", out1, out2);
...
TEST(VTASaverTest, ResConv7Test) {
  ...
  conv_test_template(1, 14, 14, 256, 1, 256, 3, 3, 256, 1, 1, 1, 1, 1, 0, 10,
                     -10, 10, 0, 32768, "vtaResConv7Test", out1, out2);
...
TEST(VTASaverTest, ResConv8Test) {
  ...
  conv_test_template(1, 14, 14, 256, 1, 512, 1, 1, 256, 1, 1, 2, 0, 1, 0, 10,
                     -10, 10, 0, 32768, "vtaResConv8Test", out1, out2);
...
TEST(VTASaverTest, ResConv9Test) {
  ...
  conv_test_template(1, 14, 14, 256, 1, 512, 3, 3, 256, 1, 1, 2, 1, 1, 0, 10,
                     -10, 10, 0, 32768, "vtaResConv9Test", out1, out2);
...
TEST(VTASaverTest, ResConv10Test) {
  ...
  conv_test_template(1, 7, 7, 512, 1, 512, 3, 3, 512, 1, 1, 1, 1, 1, 0, 10, -10,
                     10, 0, 32768, "vtaResConv10Test", out1, out2);
...
TEST(VTASaverTest, Conv_inception_5b_5x5_1__1) {
  ...
  conv_test_template(1, 7, 7, 48, 1, 128, 5, 5, 48, 1, 1, 1, 2, 1, 0, 10, -10,
                     10, 0, 32768, "Conv_inception_5b_5x5_1__1", out1, out2);
...
TEST(VTASaverTest, conv_gpu_0_conv1) {
  ...
  llvm::StringRef mainEntryName =
      "vtaFloatConvTestMainEntry"; // main function name
...
TEST(VTASaverTest, quantized_conv_gpu_0_conv1) {
  ...
  llvm::StringRef mainEntryName =
      "vtaNonVTAConvTestMainEntry"; // main function name
...
TEST(VTASaverTest, maxPoolTest) {
  ...
  llvm::StringRef mainEntryName =
      "vtaMaxPoolTestMainEntry"; // main function name
...
TEST(VTASaverTest, avgPoolTest) {
  ...
  TypeRef OT1 =
      F->getParent()->uniqueType(out1.getElementType(), out1.dims(),
                                 outType.getScale(), outType.getOffset());
  auto *avgPool =
  ...
  llvm::StringRef mainEntryName =
      "vtaAvgPoolTestMainEntry"; // main function name
...
TEST(VTASaverTest, quantizeTest) {
  ...
  llvm::StringRef mainEntryName =
      "vtaQuantizeTestMainEntry"; // main function name
...
TEST(VTASaverTest, dequantizeTest) {
  ...
  llvm::StringRef mainEntryName =
      "vtaDequantizeTestMainEntry"; // main function name
...
TEST(VTASaverTest, transposeTest) {
  ...
  llvm::StringRef mainEntryName =
      "vtaTransposeTestMainEntry"; // main function name
...
TEST(VTASaverTest, elementadd) {
  ...
  inputP0 =
      createQuantizedPlaceholder(M, bindings, &inputs0, inType0.getScale(),
                                 inType0.getOffset(), "inputP0");
  inputP1 =
      createQuantizedPlaceholder(M, bindings, &inputs1, inType1.getScale(),
  ...
  llvm::StringRef mainEntryName =
      "vtaElemAddTestMainEntry"; // main function name
...
TEST(VTASaverTest, elementsub) {
  ...
  llvm::StringRef mainEntryName =
      "vtaElemSubTestMainEntry"; // main function name
...
TEST(VTASaverTest, elementdiv) {
  ...
  llvm::StringRef mainEntryName =
      "vtaElemDivTestMainEntry"; // main function name
...
TEST(VTASaverTest, splatMax) {
  ...
  llvm::StringRef mainEntryName =
      "vtaSplatMaxTestMainEntry"; // main function name
...
TEST(VTASaverTest, softMaxTest) {
  ...
  llvm::StringRef mainEntryName =
      "vtaSoftMaxTestMainEntry"; // main function name
