...
/// A class controlling a single "STC Device", a thread of execution in
/// the IR-STC. Many STCFunctions may be added, but only one
/// inference is executed at a time.
        class STCDeviceManager : public QueueBackedDeviceManager {
            ...
            static constexpr const char *kDevicesUsedSTC =
                    "glow.devices_used.interpreter";

        public:
            explicit STCDeviceManager(const DeviceConfig &config)
                    : QueueBackedDeviceManager(config) {
                statsExporterRegistry_->incrementCounter(kDevicesUsedSTC);
                exportMemoryCounters();
            }

            ~STCDeviceManager() override {
                statsExporterRegistry_->incrementCounter(kDevicesUsedSTC, -1);
                zeroMemoryCounters();
            }
