...
STCFunction::STCFunction(std::unique_ptr<IRFunction> F,
                                         runtime::RuntimeBundle &&bundle)
        : CompiledFunction(std::move(bundle)), F_(std::move(F)) {}

STCFunction::~STCFunction() {
...
void STCFunction::collectConstants(const Module *module) {
...
void STCFunction::addConstant(std::string name, Tensor *T) {
    Tensor *newTensor = new Tensor;
    newTensor->assign(T);
    constants_[name] = newTensor;
}

Error STCFunction::execute(ExecutionContext *context) {
    BoundSTCFunction boundFunc(constants_);
...
void STCFunction::translateTraceEvents(
...
BoundSTCFunction::~BoundSTCFunction() {
...
Tensor *BoundSTCFunction::getTensor(const Value *v) const {
...
Tensor *BoundSTCFunction::getOrCreateTensor(const Value *v) {
    auto ie = externalTensors_.find(v);
...
Tensor *BoundSTCFunction::getOrCreateUnownedTensor(
...
void BoundSTCFunction::deleteTensor(const Value *v) {
    auto it = tensors_.find(v);
    if (it == tensors_.end()) {
        return;
    }
...
Error BoundSTCFunction::execute(IRFunction *F,
    ...
    // Dispatch the STC on each instruction in the program.
