...
namespace glow {
    namespace runtime {
        extern unsigned GlowSTCMemory;
    }
} // namespace glow
using namespace glow;

Expected<std::unique_ptr<CompiledFunction>>
STC::compile(Function *F, const BackendOptions &opts) const {
...
STC::compileIR(std::unique_ptr<IRFunction> IR) const {
    auto *mod = IR->getGraph()->getParent();
    auto function = compileIRWithoutConstants(std::move(IR));
    auto IFunction = static_cast<STCFunction *>(function.get());
    IFunction->collectConstants(mod);
    return function;
}

std::unique_ptr<CompiledFunction>
STC::compileIRWithoutConstants(std::unique_ptr<IRFunction> IR) const {
    ...
    auto compiledFunction =
            glow::make_unique<STCFunction>(std::move(IR), std::move(bundle));
    compiledFunction->setIRInstructionProcessingHandler(
            getIRInstructionProcessingHandler());
    return compiledFunction;
}

bool STC::isOpSupported(const NodeInfo &NI) const {
    switch (NI.getKind()) {
...
static bool checkLayout(const T &I) {
    if (I.getLayout() != NHWC) {
        report("Glow STC supports only NHWC");
        return false;
    }
    return true;
}
...
bool STC::verify(const Function &F, bool verbose) const {
    if (!F.verify(this)) {
        return false;
    }
...
bool STC::verify(const IRFunction &IR) const {
...
bool STC::shouldLower(const Node *N) const {
    switch (N->getKind()) {
        case Kinded::Kind::ConvolutionNodeKind:
        case Kinded::Kind::Convolution3DNodeKind:
        case Kinded::Kind::SparseLengthsSumNodeKind:
...
Expected<bool> STC::transformPostLowering(
        Function *F, CompilationContext &cctx,
        const glow::runtime::DeviceInfo *devInfo) const {
    LOG_SCOPE(F->getLogContext(), "STC::transformPostLowering")
...
void STC::parseBackendSpecificOptions(
        const BackendOptions &opts) const {
    auto STCMaxMemOpt =
            opts.backendSpecificOpts.find("STC-memory");
    if (STCMaxMemOpt != opts.backendSpecificOpts.end()) {
        glow::runtime::GlowSTCMemory =
                std::stoi(STCMaxMemOpt->second);
        llvm::outs() << "STC memory set to "
                     << glow::runtime::GlowSTCMemory << "\n";
    }
}
