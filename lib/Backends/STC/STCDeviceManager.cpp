...
namespace glow {
    namespace runtime {
        ...
        unsigned GlowSTCMemory = 0;
        static llvm::cl::OptionCategory
                STCBackendCat("Glow STC Backend Options");
        static llvm::cl::opt<unsigned, /* ExternalStorage */ true> STCMaxMemOpt(
                "STC-memory",
                llvm::cl::desc("STC DeviceManager maximum memory in kilobytes"),
                llvm::cl::location(GlowSTCMemory),
                llvm::cl::cat(STCBackendCat));

        DeviceManager *createSTCDeviceManager(const DeviceConfig &config) {
            if (GlowSTCMemory) {
                // Convert command line GlowSTCMemory to bytes from kilobytes.
                auto configNew = config;
                configNew.setDeviceMemory(uint64_t{GlowSTCMemory} * 1024);
                return new STCDeviceManager(configNew);
            }
            return new STCDeviceManager(config);
        }

        uint64_t STCDeviceManager::getMaximumMemory() const {
            return maxMemoryBytes_;
        }

        uint64_t STCDeviceManager::getAvailableMemory() const {
            return maxMemoryBytes_ - usedMemoryBytes_;
        }

        bool STCDeviceManager::isMemoryAvailable(uint64_t estimate) const {
            return maxMemoryBytes_ >= (usedMemoryBytes_ + estimate);
        }

        DeviceInfo STCDeviceManager::getDeviceInfo() const {
            // TODO: these may need to be tweaked depending on STC overheads.
        ...
        void STCDeviceManager::addNetworkImpl(const Module *module,
            ...
            for (const auto &func : functions) {
                ...
                if (func.second->getCompileBackendName() != STC::getName()) {
                    readyCB(module, MAKE_ERR(llvm::formatv("Failed to add network: function "
                                                           "{0} is not a STCFunction",
                                                           func.first)
                                                     .str()));
                    return;
                }
                ...
                STCFunction *function =
                        static_cast<STCFunction *>(func.second);
        ...
        void STCDeviceManager::transferStaticPlaceholderToDevice(
            ...
            for (auto functionName : it->second) {
                STCFunction *func =
                        static_cast<STCFunction *>(functions_[functionName]);
                func->addConstant(PH->getName(), T);
            }
            resultCB(Error::success());
        }

        void STCDeviceManager::evictNetworkImpl(std::string functionName,
        ...
        void STCDeviceManager::runFunctionImpl(
            ...
            if (context->getTraceContext()) {
                context->getTraceContext()->setThreadName("STC");
            }
