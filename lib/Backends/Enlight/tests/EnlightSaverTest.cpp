...
TEST(EnlightSaverTest, fcTest2_3) {
  PseudoRNG PRNG;
...
TEST(EnlightSaverTest, fcTest4) {
  ...
  outP = createPlaceholder(mod, bindings, &outEnlight1, "outP");
  OT0 = F->getParent()->uniqueType(outEnlight0.getElementType(), outEnlight0.dims());
  OT1 = F->getParent()->uniqueType(outEnlight1.getElementType(), outEnlight1.dims());
  ...
  backend->save(F, outputDir, bundleName, mainEntryName);
  outEnlight1.toBin("newtonFCTest4Golden");
