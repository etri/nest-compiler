...
#include "EnlightDeviceManager.h"
#include "Enlight.h"
...
namespace glow {
    namespace runtime {
        ...
        unsigned GlowEnlightMemory = 0;
        static llvm::cl::OptionCategory
                EnlightBackendCat("Glow Enlight Backend Options");
        static llvm::cl::opt<unsigned, /* ExternalStorage */ true> newtonMaxMemOpt(
                "newton-memory",
                llvm::cl::desc("Enlight DeviceManager maximum memory in kilobytes"),
                llvm::cl::location(GlowEnlightMemory),
                llvm::cl::cat(EnlightBackendCat));

        DeviceManager *createEnlightDeviceManager(const DeviceConfig &config) {
            if (GlowEnlightMemory) {
                // Convert command line GlowEnlightMemory to bytes from kilobytes.
                auto configNew = config;
                configNew.setDeviceMemory(uint64_t{GlowEnlightMemory} * 1024);
                return new EnlightDeviceManager(configNew);
            }
            return new EnlightDeviceManager(config);
        }

        uint64_t EnlightDeviceManager::getMaximumMemory() const {
            return maxMemoryBytes_;
        }

        uint64_t EnlightDeviceManager::getAvailableMemory() const {
            return maxMemoryBytes_ - usedMemoryBytes_;
        }

        bool EnlightDeviceManager::isMemoryAvailable(uint64_t estimate) const {
            return maxMemoryBytes_ >= (usedMemoryBytes_ + estimate);
        }

        DeviceInfo EnlightDeviceManager::getDeviceInfo() const {
        ...
        void EnlightDeviceManager::addNetworkImpl(const Module *module,
            ...
            for (const auto &func : functions) {
                ...
                if (func.second->getCompileBackendName() != Enlight::getName()) {
                    readyCB(module, MAKE_ERR(llvm::formatv("Failed to add network: function "
                                                           "{0} is not a EnlightFunction",
                                                           func.first)
                                                     .str()));
                    return;
                }
                ...
                EnlightFunction *function =
                        static_cast<EnlightFunction *>(func.second);
        ...
        void EnlightDeviceManager::transferStaticPlaceholderToDevice(
            ...
            for (auto functionName : it->second) {
                EnlightFunction *func =
                        static_cast<EnlightFunction *>(functions_[functionName]);
                func->addConstant(PH->getName(), T);
            }
            resultCB(Error::success());
        }

        void EnlightDeviceManager::evictNetworkImpl(std::string functionName,
        ...
        void EnlightDeviceManager::runFunctionImpl(
            ...
            if (context->getTraceContext()) {
                context->getTraceContext()->setThreadName("Enlight");
            }
