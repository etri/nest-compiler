...
namespace glow {
namespace runtime {
extern unsigned GlowEnlightMemory;
}
} // namespace glow
...
Enlight::compile(Function *F, const BackendOptions &opts) const {
...
Enlight::compileIR(std::unique_ptr<IRFunction> IR) const {
  auto *mod = IR->getGraph()->getParent();
  auto function = compileIRWithoutConstants(std::move(IR));
  auto IFunction = static_cast<EnlightFunction *>(function.get());
  IFunction->collectConstants(mod);
  return function;
}

std::unique_ptr<CompiledFunction>
Enlight::compileIRWithoutConstants(std::unique_ptr<IRFunction> IR) const {
  ...
  auto compiledFunction =
      glow::make_unique<EnlightFunction>(std::move(IR), std::move(bundle));
  compiledFunction->setIRInstructionProcessingHandler(
      getIRInstructionProcessingHandler());
  return compiledFunction;
}

bool Enlight::isOpSupported(const NodeInfo &NI) const {
  switch (NI.getKind()) {
...
static bool checkLayout(const T &I) {
  if (I.getLayout() != NHWC) {
    report("Glow Enlight supports only NHWC");
    return false;
  }
  return true;
}
...
bool Enlight::verify(const Function &F, bool verbose) const {
  if (!F.verify(this)) {
    return false;
  }
...
bool Enlight::verify(const IRFunction &IR) const {
...
bool Enlight::shouldLower(const Node *N) const {
  switch (N->getKind()) {
...
Expected<bool> Enlight::transformPostLowering(
    Function *F, CompilationContext &cctx,
    const glow::runtime::DeviceInfo *devInfo) const {
  LOG_SCOPE(F->getLogContext(), "Enlight::transformPostLowering")
...
void Enlight::parseBackendSpecificOptions(
    const BackendOptions &opts) const {
  auto EnlightMaxMemOpt =
      opts.backendSpecificOpts.find("Enlight-memory");
  if (EnlightMaxMemOpt != opts.backendSpecificOpts.end()) {
    glow::runtime::GlowEnlightMemory =
        std::stoi(EnlightMaxMemOpt->second);
    llvm::outs() << "Enlight memory set to "
                 << glow::runtime::GlowEnlightMemory << "\n";
  }
}
