...
namespace glow {
    namespace runtime {
        ...
        unsigned GlowNewtonMemory = 0;
        static llvm::cl::OptionCategory
                NewtonBackendCat("Glow Newton Backend Options");
        static llvm::cl::opt<unsigned, /* ExternalStorage */ true> newtonMaxMemOpt(
                "newton-memory",
                llvm::cl::desc("Newton DeviceManager maximum memory in kilobytes"),
                llvm::cl::location(GlowNewtonMemory),
                llvm::cl::cat(NewtonBackendCat));

        DeviceManager *createNewtonDeviceManager(const DeviceConfig &config) {
            if (GlowNewtonMemory) {
                // Convert command line GlowNewtonMemory to bytes from kilobytes.
                auto configNew = config;
                configNew.setDeviceMemory(uint64_t{GlowNewtonMemory} * 1024);
                return new NewtonDeviceManager(configNew);
            }
            return new NewtonDeviceManager(config);
        }

        uint64_t NewtonDeviceManager::getMaximumMemory() const {
            return maxMemoryBytes_;
        }

        uint64_t NewtonDeviceManager::getAvailableMemory() const {
            return maxMemoryBytes_ - usedMemoryBytes_;
        }

        bool NewtonDeviceManager::isMemoryAvailable(uint64_t estimate) const {
            return maxMemoryBytes_ >= (usedMemoryBytes_ + estimate);
        }

        DeviceInfo NewtonDeviceManager::getDeviceInfo() const {
        ...
        void NewtonDeviceManager::addNetworkImpl(const Module *module,
            ...
            for (const auto &func : functions) {
                ...
                if (func.second->getCompileBackendName() != Newton::getName()) {
                    readyCB(module, MAKE_ERR(llvm::formatv("Failed to add network: function "
                                                           "{0} is not a NewtonFunction",
                                                           func.first)
                                                     .str()));
                    return;
                }
                ...
                NewtonFunction *function =
                        static_cast<NewtonFunction *>(func.second);
        ...
        void NewtonDeviceManager::transferStaticPlaceholderToDevice(
            ...
            for (auto functionName : it->second) {
                NewtonFunction *func =
                        static_cast<NewtonFunction *>(functions_[functionName]);
                func->addConstant(PH->getName(), T);
            }
            resultCB(Error::success());
        }

        void NewtonDeviceManager::evictNetworkImpl(std::string functionName,
        ...
        void NewtonDeviceManager::runFunctionImpl(
            ...
            if (context->getTraceContext()) {
                context->getTraceContext()->setThreadName("Newton");
            }
