...
NewtonFunction::NewtonFunction(std::unique_ptr<IRFunction> F,
...
void NewtonFunction::collectConstants(const Module *module) {
...
void NewtonFunction::addConstant(std::string name, Tensor *T) {
    Tensor *newTensor = new Tensor;
    newTensor->assign(T);
    constants_[name] = newTensor;
}

Error NewtonFunction::execute(ExecutionContext *context) {
    BoundNewtonFunction boundFunc(constants_);
...
void NewtonFunction::translateTraceEvents(
...
BoundNewtonFunction::~BoundNewtonFunction() {
...
Tensor *BoundNewtonFunction::getTensor(const Value *v) const {
...
Tensor *BoundNewtonFunction::getOrCreateTensor(const Value *v) {
    auto ie = externalTensors_.find(v);
...
Tensor *BoundNewtonFunction::getOrCreateUnownedTensor(
...
void BoundNewtonFunction::deleteTensor(const Value *v) {
    auto it = tensors_.find(v);
    if (it == tensors_.end()) {
        return;
    }
...
Error BoundNewtonFunction::execute(IRFunction *F,
