...
DeviceManager *createRelayDeviceManager(const DeviceConfig &config) {
  if (GlowRelayMemory) {
    // Convert command line GlowRelayMemory to bytes from kilobytes.
    auto configNew = config;
    configNew.setDeviceMemory(uint64_t{GlowRelayMemory} * 1024);
    return new RelayDeviceManager(configNew);
  }
  return new RelayDeviceManager(config);
}

uint64_t RelayDeviceManager::getMaximumMemory() const {
  return maxMemoryBytes_;
}

uint64_t RelayDeviceManager::getAvailableMemory() const {
  return maxMemoryBytes_ - usedMemoryBytes_;
}

bool RelayDeviceManager::isMemoryAvailable(uint64_t estimate) const {
  return maxMemoryBytes_ >= (usedMemoryBytes_ + estimate);
}

DeviceInfo RelayDeviceManager::getDeviceInfo() const {
  // TODO: these may need to be tweaked depending on Relay overheads.
...
void RelayDeviceManager::addNetworkImpl(const Module *module,
  ...
  for (const auto &func : functions) {
    ...
    if (func.second->getCompileBackendName() != Relay::getName()) {
      readyCB(module, MAKE_ERR(llvm::formatv("Failed to add network: function "
                                             "{0} is not a RelayFunction",
                                             func.first)
                                   .str()));
      return;
    }
...
void RelayDeviceManager::transferStaticPlaceholderToDevice(
  ...
  for (auto functionName : it->second) {
    RelayFunction *func =
        static_cast<RelayFunction *>(functions_[functionName]);
...
void RelayDeviceManager::runFunctionImpl(
  ...
  if (context->getTraceContext()) {
    context->getTraceContext()->setThreadName("Relay");
  }
