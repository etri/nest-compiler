...
string CCodeGenerator::getDataCountStr(const TypeRef value)
{
    ...
    // size_t size = (size_t)tensor.getType().getElementSize(tensor.getElementType());
    size_t size = 4; //check!
...
void CCodeGenerator::generateDeclaration(Loader &loader, string& wfilec, Node& n)
{
...
//            wfilec.append( "//Relu\n");
        ...
        //string in1varname = string("var").append(node->getInput().getNode()->getName());
        //string biasname = string("var").append(node->getBias().getNode()->getName());
...
//        BatchNormalizationNode* node = (BatchNormalizationNode *)&n;
        ...
        kernelVecVar = string("kernel").append(to_string((size_t)node->getKernels().data()));
        wfilec.append("vector<size_t> " + kernelVecVar  +";\n");

        strideVecVar = string("stride").append(to_string((size_t)node->getStrides().data()));
        wfilec.append("vector<size_t> " + strideVecVar  +";\n");
...
void CCodeGenerator::allocConstVar(string& wfilec, ofstream* wfileb, Constant* node)
{
...
void CCodeGenerator::generateInitialization(Loader &loader, string& wfilec, ofstream* wfileb, Node& n) {
...
//    }else if (n == nodeList.back() ||!kindName.compare("Save")) {
        ...
        wfilec.append("\n}\n");
        cout << "===> 2. Memory allocation & variable initialization part has been generated.\n";
        return;
    }
...
//            }
    ...
    if(!kindName.compare("Add")) {
        ...
        string shuffleVar = string("shuffle").append(to_string((size_t) node->getShuffle().data()));
...
//        wfilec.append("\t//group: " + std::to_string(group) +  ";\n");
        ...
        wfilec.append("\treadFileBin.read((char *)&" + channelVar + ", sizeof(unsigned int));\n");

        float epsilon = node->getEpsilon();
        wfileb->write((char *)(&epsilon), sizeof(float));
        wfilec.append("\treadFileBin.read((char *)&" + epsilonVar + ", sizeof(float));\n");
...
void CCodeGenerator::generateInference(Loader &loader, string inputName, string& wfilec, string inAryStr, Node& n) {
...
//    } else if(n == *nodeList.getPrevNode(lastNode)) {
    ...
    if(!kindName.compare("Add")) {
        ...
        wfilec.append("\t//Transpose\n");
        wfilec.append("\ttranspose(*" + in0varname + ", " + shuffleVar + ", *" + outvarname +");\n\n");
...
//        std::cout << "group = " << group << std::endl;
 ...
 + node->getResult().getType()->getElementTypeName().str() + getDataArrayStr(node->getResult().getType()) + "));\n");
        ...
        wfilec.append("\t//Matmul\n");
        string in1varname = string("var").append(n.getNthInput(1).getNode()->getName());
        wfilec.append("\tmatmul(*" +in0varname+ ", *" + in1varname + ", *" + outvarname + ");\n\n");
        ...
        string Avarname =
                string("var").append(node->getA().getNode()->getName());

        string Bvarname =
                string("var").append(node->getB().getNode()->getName());

        string Cvarname =
                string("var").append(node->getC().getNode()->getName());
...
//          cout << to_string(transB) <<endl;
        ...
        string funcStr = "\tgemm(*" + Avarname + ", *" + Bvarname + ", *" +
                         Cvarname + ", " + to_string(alpha) + ", " + to_string(beta) + ", " +
                         to_string(transA) + ", " + to_string(transB)+ ", *" + outvarname + ");\n\n";
...
//          cout << getDataArrayStr(node->getC().getType()) << endl;
        ...
        wfilec.append("\tfullyconnected(*" + in0varname + ", *" + weightvarname + ", *" + biasname + ", *" +
                      outvarname + ");\n\n");
        ...
        wfilec.append("\t//BatchNormalization\n");
        wfilec.append("\tbatchNorm(*" + in0varname+ ", *" + node->getScale().getNode()->getVarName() + ", *" + node->getBias().getNode()->getVarName() + ", *" + \
			node->getMean().getNode()->getVarName() + ", *" + node->getVar().getNode()->getVarName() + ", " + channelVar + ", " +  epsilonVar + ", " + momentumVar \
			+ ", *" + outvarname +");\n\n");
...
//            std::cout << getTotalSizeStr(v) << std::endl;
    ...
    if(isLast == 1) {
        wfilec.append("\tmemcpy(result," + outvarname + ", " + getTotalSizeStr(n.getNthResult(0)) + ");\n");
    }

}

void CCodeGenerator::generateFree(Loader &loader, string inputName, string& wfilec, string inAryStr, Node& n)
{
...
//    } else if(n == *nodeList.getPrevNode(lastNode)) {
    ...
    if (!kindName.compare("Add")) {
        ...
        string weightvarname = string("var").append(node->getWeights().getNode()->getName());
...
string CCodeGenerator::replaceAll(std::string &str, std::string pattern, std::string replace) {
  string result = str;
...
void CCodeGenerator::generateCodeFromModels(Loader &loader) {
    ...
    cout << "first node size = " << getDataArrayStr(firstNode->getNthInput(0).getType()) << endl;
    cout << "last node size = " << getDataArrayStr(lastNode->getNthInput(0).getType()) << endl;
...
//  const auto &lastNode = nodes.back();
    ...
    if(writeFileH.is_open()) {
        ...
        writeFileH << "#include <vector>" << endl << "#include <iostream>" << endl << "#include <fstream>" << endl << "#include \"library.h\"" << endl << endl;
...
//            if((i == 0 && j == 0) || (i == level-1 && j == bfs[i].size()-2)) continue;
