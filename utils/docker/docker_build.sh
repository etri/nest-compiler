...
if [[ $# -ne 2 ]]
then
    echo "Direction: ./docker_build.sh NEST_COMPILER_HOME_DIR BUILD_DIR_NAME"
    exit
fi
...
docker create -i -v $NEST_COMPILER_HOME:/root/dev/nestc_workspace --name=nest_container nest_compiler:0.0 /bin/bash
...
docker exec nest_container sh -c "cd nestc_workspace; ls;rm -rf $2/*; cd $2; ls; cmake -G Ninja ../ -DCMAKE_BUILD_TYPE=Release"
echo "#######Docker Nest Compiler Build#######"
docker exec nest_container sh -c "cd nestc_workspace; cd $2; ninja all;"
docker container rm nest_container -f
