...
of compile time by allowing the users of NEST-C to compile a network model Ahead Of
Time.
...
the model in a standalone mode. NEST-C has multiple backends which can run models
but not all of them are capable of saving a compiled model.

The backends of NEST-C to generate bundles are **CPU** and **EVTA** bundles, where the
...
When building NEST-C you can choose to build the **ResNet50** and the **LeNetMnist**
examples by enabling the `GLOW_WITH_BUNDLES` option as below:

```
mkdir build_Debug
cd build_Debug 
cmake -G Ninja -DCMAKE_BUILD_TYPE=Debug -DGLOW_WITH_BUNDLES=ON ../nest-compiler
ninja all
```
...
The **model-compiler** front-end tool is the main NEST-C tool used to compile **ONNX**, **Caffe2** and
...
The command used to build a bundle using the EVTA backend is the following:
```
model-compiler -backend=VTA -model=<model-path> -emit-bundle=<bundle-dir>
```
...
For example, to compile the **ResNet50** model provided by NEST-C in Caffe2 format, you can use
...
Similarly, to compile the **LeNetMnist** model provided by NEST-C in Caffe2 format, you can use
...
NEST-C support for out-of-the-box quantized models is a work in progress mainly because the
quantized tensors and operators are not well standardized in formats like Caffe2 and ONNX.

The way NEST-C produces a quantized bundle is by taking a floating-point model and converting
...
Since the CPU backend is based on LLVM, the NEST-C tools can be used to
...
for the native architecture - the one which is running NEST-C). Below you
...
size (symbol) associated with the batch size of the model. NEST-C cannot compile a
...
graph after all the transformations and optimizations performed by NEST-C
...
A very popular tool for visualizing the original model before compiling with NEST-C is **Netron** which
has:
...
The NEST-C compiler currently has support only for Caffe2, ONNX and TensorFlowLite model formats. Since a lot of popular
...
After converting the model to ONNX you can build a bundle using the NEST-C **model-compiler** tool with the command:
```
...
one needs to modify an ONNX model in order to circumvent a limitation of the NEST-C
