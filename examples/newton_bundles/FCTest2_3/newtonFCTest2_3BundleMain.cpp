...
#include "newtonFCTest2_3MainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
static void loadImagesAndPreprocess(std::vector<std::string> &filenames,
                                    int8_t *&resultT, size_t *resultDims) {
  if(filenames.size() ==0)
  {
    filenames.push_back("newtonFCTest2_3Input.bin");
  }
...
static void dumpInferenceResults(const BundleConfig &config,
  ...
  for (int i = 0 ; i < 1000; i++){
    fos.write((const char *)&results[i], 4);
  }
  fos.close();
}
...
static int8_t *initMutableWeightVars(const BundleConfig &config) {
  ...
  size_t imageDataSizeInBytes =
          inputDims[0] * inputDims[1] * sizeof(float);
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("newtonFCTest2_3Bundle.weights.bin", newtonFCTest2_3Bundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(newtonFCTest2_3Bundle_config);
  int8_t *activationsAddr = initActivations(newtonFCTest2_3Bundle_config);


  newtonFCTest2_3MainEntry_load_module((float *)constantWeightVarsAddr);
  // Perform the computation.
  int errCode =
      newtonFCTest2_3MainEntry((float*)constantWeightVarsAddr, (float*)mutableWeightVarsAddr, (float*)activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(newtonFCTest2_3Bundle_config, mutableWeightVarsAddr);
