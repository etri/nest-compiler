...
#include "newtonFCTest4MainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
static void loadImagesAndPreprocess(std::vector<std::string> &filenames,
  ...
  if(filenames.size() ==0)
  {
    filenames.push_back("newtonFCTest4Input.bin");
  }
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("newtonFCTest4Bundle.weights.bin", newtonFCTest4Bundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(newtonFCTest4Bundle_config);
  int8_t *activationsAddr = initActivations(newtonFCTest4Bundle_config);


  newtonFCTest4MainEntry_load_module((float *)constantWeightVarsAddr);
  // Perform the computation.
  int errCode =
      newtonFCTest4MainEntry((float*)constantWeightVarsAddr, (float*)mutableWeightVarsAddr, (float*)activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(newtonFCTest4Bundle_config, mutableWeightVarsAddr);
