...
#include "newtonFCTest1MainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
static void loadImagesAndPreprocess(std::vector<std::string> &filenames,
  ...
  if(filenames.size() ==0)
  {
    filenames.push_back("newtonFCTest1Input.bin");
  }
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("newtonFCTest1Bundle.weights.bin", newtonFCTest1Bundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(newtonFCTest1Bundle_config);
  int8_t *activationsAddr = initActivations(newtonFCTest1Bundle_config);


  newtonFCTest1MainEntry_load_module((float *)constantWeightVarsAddr);
  // Perform the computation.
  int errCode =
      newtonFCTest1MainEntry((float*)constantWeightVarsAddr, (float*)mutableWeightVarsAddr, (float*)activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(newtonFCTest1Bundle_config, mutableWeightVarsAddr);
