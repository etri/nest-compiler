...
#include "newtonFCTest2MainEntry.h"
#include <vector>
#include <string>
#include <assert.h>
#include <inttypes.h>
...
static void loadImagesAndPreprocess(std::vector<std::string> &filenames,
  ...
  if(filenames.size() ==0)
  {
    filenames.push_back("newtonFCTest2Input.bin");
  }
...
int main(int argc, char **argv) {
  ...
  int8_t *constantWeightVarsAddr =
          initConstantWeights("newtonFCTest2Bundle.weights.bin", newtonFCTest2Bundle_config);
  int8_t *mutableWeightVarsAddr = initMutableWeightVars(newtonFCTest2Bundle_config);
  int8_t *activationsAddr = initActivations(newtonFCTest2Bundle_config);


  newtonFCTest2MainEntry_load_module((float *)constantWeightVarsAddr);
  // Perform the computation.
  int errCode =
      newtonFCTest2MainEntry((float*)constantWeightVarsAddr, (float*)mutableWeightVarsAddr, (float*)activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  dumpInferenceResults(newtonFCTest2Bundle_config, mutableWeightVarsAddr);
