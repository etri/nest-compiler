...
//  uint8_t *constantWeightVarsAddr = initConstantWeights("inception.weights.bin", inception_config);
  uint8_t *mutableWeightVarsAddr = initMutableWeightVars(inception_config, "data_0");
  //uint8_t *activationsAddr = initActivations(inception_config);

  // Perform the computation.
  inception_load_module(0);
  
  int errCode =
      inception(0, mutableWeightVarsAddr,0);  
...
//  dumpInferenceResults(resnet18v2_config, mutableWeightVarsAddr, "resnetv22_dense0_fwd__1");
  int maxIdx = dumpInferenceResults(inception_config, mutableWeightVarsAddr, "prob_1");
