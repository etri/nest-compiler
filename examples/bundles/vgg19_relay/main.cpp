...
int main(int argc, char **argv) {
  ...
  //uint8_t *constantWeightVarsAddr = initConstantWeights("vgg19.weights.bin", vgg19_config);
  uint8_t *mutableWeightVarsAddr = initMutableWeightVars(vgg19_config, "data_0");
  //uint8_t *activationsAddr = initActivations(vgg19_config);

  // Perform the computation.
  vgg19_load_module(0);
  int errCode =
      vgg19(0, mutableWeightVarsAddr, 0);
...
//  dumpInferenceResults(resnet18v2_config, mutableWeightVarsAddr, "resnetv22_dense0_fwd__1");
  int maxIdx = dumpInferenceResults(vgg19_config, mutableWeightVarsAddr, "prob_1");
