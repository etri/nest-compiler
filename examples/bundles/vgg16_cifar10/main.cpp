...
int main(int argc, char **argv) {
  ...
  uint8_t *constantWeightVarsAddr =
      initConstantWeights("vgg16_cifar10.weights.bin", vgg16_cifar10_config);
  uint8_t *mutableWeightVarsAddr = initMutableWeightVars(vgg16_cifar10_config, "input"); //"data_0");
  uint8_t *activationsAddr = initActivations(vgg16_cifar10_config);

  // Perform the computation.
  int errCode =
      vgg16_cifar10(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
...
//  dumpInferenceResults(resnet18v2_config, mutableWeightVarsAddr, "resnetv22_dense0_fwd__1");
  int maxIdx = dumpInferenceResults(vgg16_cifar10_config, mutableWeightVarsAddr, "output");
