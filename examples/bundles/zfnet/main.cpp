...
int main(int argc, char **argv) {
  ...
  uint8_t *constantWeightVarsAddr =
      initConstantWeights("zfnet.weights.bin", zfnet_config);
  uint8_t *mutableWeightVarsAddr = initMutableWeightVars(zfnet_config, "gpu_0_data_0");
  uint8_t *activationsAddr = initActivations(zfnet_config);

  // Perform the computation.
  int errCode =
      zfnet(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
...
//  dumpInferenceResults(resnet18v2_config, mutableWeightVarsAddr, "resnetv22_dense0_fwd__1");
  int maxIdx = dumpInferenceResults(zfnet_config, mutableWeightVarsAddr, "gpu_0_softmax_1");
