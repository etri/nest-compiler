...
int main(int argc, char **argv) {
  ...
  //uint8_t *constantWeightVarsAddr =      initConstantWeights("mnasnet.weights.bin", mnasnet_config);
  uint8_t *mutableWeightVarsAddr = initMutableWeightVars(mnasnet_config);
  //uint8_t *activationsAddr = initActivations(mnasnet_config);

  // Perform the computation.
  mnasnet_load_module(0);
  
  int errCode =
      mnasnet(0, mutableWeightVarsAddr,0);  
  ...
  // Report the results.
  int maxIdx = dumpInferenceResults(mnasnet_config, mutableWeightVarsAddr);
