...
int main(int argc, char **argv) {
  ...
  //uint8_t *constantWeightVarsAddr = initConstantWeights("squeezenet.weights.bin", squeezenet_config);
  uint8_t *mutableWeightVarsAddr = initMutableWeightVars(squeezenet_config, "data_0");
  //uint8_t *activationsAddr = initActivations(squeezenet_config);

  // Perform the computation.
  squeezenet_load_module(0);
  int errCode =
      squeezenet(0, mutableWeightVarsAddr, 0);
...
//  dumpInferenceResults(resnet18v2_config, mutableWeightVarsAddr, "resnetv22_dense0_fwd__1");
  int maxIdx = dumpInferenceResults(squeezenet_config, mutableWeightVarsAddr, "softmaxout_1");
