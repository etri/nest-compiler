...
int main(int argc, char **argv) {
  ...
  uint8_t *constantWeightVarsAddr =
      initConstantWeights("googlenet.weights.bin", googlenet_config);
  uint8_t *mutableWeightVarsAddr = initMutableWeightVars(googlenet_config, "data_0");
  uint8_t *activationsAddr = initActivations(googlenet_config);

  // Perform the computation.
  int errCode =
      googlenet(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
...
//  dumpInferenceResults(resnet18v2_config, mutableWeightVarsAddr, "resnetv22_dense0_fwd__1");
  int maxIdx = dumpInferenceResults(googlenet_config, mutableWeightVarsAddr, "prob_1");
