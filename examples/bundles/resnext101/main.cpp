...
int main(int argc, char **argv) {
  ...
  uint8_t *constantWeightVarsAddr =
      initConstantWeights("resnext101.weights.bin", resnext101_config);
  uint8_t *mutableWeightVarsAddr = initMutableWeightVars(resnext101_config);
  uint8_t *activationsAddr = initActivations(resnext101_config);

  // Perform the computation.
  int errCode =
      resnext101(constantWeightVarsAddr, mutableWeightVarsAddr, activationsAddr);
  if (errCode != GLOW_SUCCESS) {
    printf("Error running bundle: error code %d\n", errCode);
  }

  // Report the results.
  int maxIdx = dumpInferenceResults(resnext101_config, mutableWeightVarsAddr);
